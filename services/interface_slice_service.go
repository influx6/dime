...
// InterfaceSliceDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type InterfaceSliceDataWriterFunc func([]interface{}) bool
...
type InterfaceSliceFromByteAdapter func(CancelContext, <-chan []byte) <-chan []interface{}
...
// InterfaceSliceCollect defines a function which returns a channel where the items of the incoming channel
...
// InterfaceSliceMutate defines a function which returns a channel where the items of the incoming channel
...
// InterfaceSliceView defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func InterfaceSliceView(ctx CancelContext, waitTime time.Duration, viewFn func([]interface{}), in <-chan []interface{}) <-chan []interface{} {
	res := make(chan []interface{}, 0)
...
// InterfaceSliceSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func InterfaceSliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []interface{}) chan<- []interface{} {
	res := make(chan []interface{}, 0)
...
// InterfaceSliceWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func InterfaceSliceWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- []interface{}) InterfaceSliceDataWriterFunc {
...
// InterfaceSliceWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func InterfaceSliceWriterFuncTo(ctx CancelContext, in chan<- []interface{}) InterfaceSliceDataWriterFunc {
	return func(val []interface{}) bool {
		select {
		case <-ctx.Done():
			return false
...
// InterfaceSliceSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func InterfaceSliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]interface{}) bool, in chan<- []interface{}) chan<- []interface{} {
	res := make(chan []interface{}, 0)
...
// InterfaceSliceSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func InterfaceSliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]interface{}) []interface{}, in chan<- []interface{}) chan<- []interface{} {
	res := make(chan []interface{}, 0)
...
// InterfaceSliceSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func InterfaceSliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]interface{}), in chan<- []interface{}) chan<- []interface{} {
	res := make(chan []interface{}, 0)
...
// InterfaceSliceFilter defines a function which returns a channel where the items of the incoming channel
...
// InterfaceSliceCollectUntil defines a function which returns a channel where the items of the incoming channel
...
// InterfaceSliceMergeWithoutOrder merges the incoming data from the []interface{} into a single stream of []interface{},
...
// InterfaceSliceMergeInOrder merges the incoming data from the []interface{} into a single stream of []interface{},
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// InterfaceSliceCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// InterfaceSliceCombineParitallyWithoutOrder makes the following guarantees:
...
// InterfaceSliceCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// InterfaceSliceCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// InterfaceSliceCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// InterfaceSliceCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// InterfaceSliceCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// InterfaceSliceDistributor delivers messages to subscription channels which it manages internal,
...
type InterfaceSliceDistributor struct {
	running             int64
...
// NewInterfaceSliceDistributor returns a new instance of a InterfaceSliceDistributor.
func NewInterfaceSliceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *InterfaceSliceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &InterfaceSliceDistributor{
...
func (d *InterfaceSliceDistributor) PublishDeadline(message []interface{}, dur time.Duration) {
...
func (d *InterfaceSliceDistributor) Publish(message []interface{}) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *InterfaceSliceDistributor) Subscribe(sub chan<- []interface{}) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *InterfaceSliceDistributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *InterfaceSliceDistributor) Stop() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *InterfaceSliceDistributor) manage() {
...
// MonoInterfaceSliceService defines a interface for underline systems which want to communicate like
...
// @iface
type MonoInterfaceSliceService interface {
