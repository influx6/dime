...
func TestUInt32Collect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.UInt32Collect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan uint32, 0)
		outgoing := services.UInt32Collect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUInt32PartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan uint32, 0)
		outgoing := services.UInt32PartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan uint32, 0)
		outgoing := services.UInt32PartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUInt32Mutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt32Mutate(ctx, 2*time.Millisecond, func(item uint32) uint32 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan uint32, 0)
		outgoing := services.UInt32Mutate(ctx, 10*time.Millisecond, func(item uint32) uint32 {
			return item
		}, incoming)
...
func TestUInt32View(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt32View(ctx, 2*time.Millisecond, func(item uint32) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.UInt32View(ctx, 10*time.Millisecond, func(item uint32) {
			inview <- item
		}, incoming)
...
func TestUInt32Filter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt32Filter(ctx, 2*time.Millisecond, func(item uint32) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan uint32, 0)
		outgoing := services.UInt32Filter(ctx, 10*time.Millisecond, func(item uint32) bool {
			return true
		}, incoming)
...
func TestUInt32CollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.UInt32CollectUntil(ctx, 2*time.Millisecond, func(item []uint32) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan uint32, 0)
		outgoing := services.UInt32CollectUntil(ctx, 10*time.Millisecond, func(items []uint32) bool {
			return len(items) == 2
		}, incoming)
...
func TestUInt32MergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt32MergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UInt32MergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt32MergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt32MergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt32MergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt32CombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt32CombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.UInt32CombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt32CombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt32CombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UInt32CombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt32CombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt32CombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt32CombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt32CombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt32CombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt32CombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt32Distributor(t *testing.T) {
	...
	case initial := <-incoming:
		...
		if !isUInt32Equal(initial, <-incoming2) {
		...
		if !isUInt32Equal(initial, <-incoming3) {
...
func isUInt32Equal(item1, item2 uint32) bool {
...
func isUInt32EqualSlice(item1 []uint32, item2 []uint32) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isUInt32Equal(item, item2[index]) {
			return false
		}
	}
