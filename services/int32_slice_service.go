...
// method.
func Int32SliceSink(ctx CancelContext, waitTime time.Duration, in chan<- []int32) chan<- []int32 {
	res := make(chan []int32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int32SliceSinkFilter(ctx CancelContext, waitTime time.Duration, filterFn func([]int32) bool, in chan<- []int32) chan<- []int32 {
	res := make(chan []int32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int32SliceSinkMutate(ctx CancelContext, waitTime time.Duration, mutateFn func([]int32) []int32, in chan<- []int32) chan<- []int32 {
	res := make(chan []int32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int32SliceSinkView(ctx CancelContext, waitTime time.Duration, viewFn func([]int32), in chan<- []int32) chan<- []int32 {
	res := make(chan []int32, 0)
...
func NewInt32SliceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *Int32SliceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	return &Int32SliceDistributor{
...
func (d *Int32SliceDistributor) Start() {
	if atomic.LoadInt64(&d.running) != 0 {
		return
	}
