...
// method.
func Int16SliceSink(ctx CancelContext, waitTime time.Duration, in chan<- []int16) chan<- []int16 {
	res := make(chan []int16, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int16SliceSinkFilter(ctx CancelContext, waitTime time.Duration, filterFn func([]int16) bool, in chan<- []int16) chan<- []int16 {
	res := make(chan []int16, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int16SliceSinkMutate(ctx CancelContext, waitTime time.Duration, mutateFn func([]int16) []int16, in chan<- []int16) chan<- []int16 {
	res := make(chan []int16, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int16SliceSinkView(ctx CancelContext, waitTime time.Duration, viewFn func([]int16), in chan<- []int16) chan<- []int16 {
	res := make(chan []int16, 0)
...
func NewInt16SliceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *Int16SliceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	return &Int16SliceDistributor{
...
func (d *Int16SliceDistributor) Start() {
	if atomic.LoadInt64(&d.running) != 0 {
		return
	}
