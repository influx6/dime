...
func TestInt64SliceCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []int64, 0)
		outgoing := services.Int64SliceCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan []int64, 0)
		outgoing := services.Int64SliceCollect(context.Background(), 10*time.Millisecond, incoming)

		go func() {
			for i := 3; i > 0; i-- {

				incoming <- []int64{1, 2, 3}
...
func TestInt64SlicePartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []int64, 0)
		outgoing := services.Int64SlicePartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan []int64, 0)
		outgoing := services.Int64SlicePartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestInt64SliceMutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int64SliceMutate(ctx, 2*time.Millisecond, func(item []int64) []int64 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan []int64, 0)
		outgoing := services.Int64SliceMutate(ctx, 10*time.Millisecond, func(item []int64) []int64 {
			if len(item) > 2 {
				return item[0:2]
			} else {
				return item
...
func TestInt64SliceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int64SliceView(ctx, 2*time.Millisecond, func(item []int64) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []int64, 0)
		inview := make(chan []int64, 0)

		outgoing := services.Int64SliceView(ctx, 10*time.Millisecond, func(item []int64) {
			inview <- item
		}, incoming)
...
func TestInt64SliceFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int64SliceFilter(ctx, 2*time.Millisecond, func(item []int64) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []int64, 0)
		outgoing := services.Int64SliceFilter(ctx, 10*time.Millisecond, func(item []int64) bool {
			return true
		}, incoming)
...
func TestInt64SliceCollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.Int64SliceCollectUntil(ctx, 2*time.Millisecond, func(item [][]int64) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan []int64, 0)
		outgoing := services.Int64SliceCollectUntil(ctx, 10*time.Millisecond, func(items [][]int64) bool {
			return len(items) == 2
		}, incoming)
...
func TestInt64SliceMergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64SliceMergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		incoming2 := make(chan []int64, 0)
		defer close(incoming2)

		incoming3 := make(chan []int64, 0)
		defer close(incoming3)

		outgoing := services.Int64SliceMergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64SliceMergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64SliceMergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int64SliceMergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64SliceCombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64SliceCombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.Int64SliceCombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64SliceCombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64SliceCombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Int64SliceCombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64SliceCombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64SliceCombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int64SliceCombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64SliceCombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int64SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64SliceDistributor(t *testing.T) {
	dist := services.NewInt64SliceDistributor(0, 1*time.Second)

	incoming := make(chan []int64, 1)
	incoming2 := make(chan []int64, 1)
	incoming3 := make(chan []int64, 1)
	...
	case initial := <-incoming:
		...
		if !isInt64SliceEqualSlice(initial, <-incoming2) {
		...
		if !isInt64SliceEqualSlice(initial, <-incoming3) {
...
func isInt64SliceEqualSlice(item1 []int64, item2 []int64) bool {
...
func isInt64SliceEqualDoubleSlice(item1 [][]int64, item2 [][]int64) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isInt64SliceEqualSlice(item, item2[index]) {
			return false
		}
	}
