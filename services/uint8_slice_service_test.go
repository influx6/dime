...
func TestUInt8SliceCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.UInt8SliceCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan []uint8, 0)
		outgoing := services.UInt8SliceCollect(context.Background(), 10*time.Millisecond, incoming)

		go func() {
			for i := 3; i > 0; i-- {

				incoming <- []uint8{1, 2, 3}
...
func TestUInt8SlicePartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []uint8, 0)
		outgoing := services.UInt8SlicePartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan []uint8, 0)
		outgoing := services.UInt8SlicePartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUInt8SliceMutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt8SliceMutate(ctx, 2*time.Millisecond, func(item []uint8) []uint8 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan []uint8, 0)
		outgoing := services.UInt8SliceMutate(ctx, 10*time.Millisecond, func(item []uint8) []uint8 {
			if len(item) > 2 {
				return item[0:2]
			} else {
				return item
...
func TestUInt8SliceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt8SliceView(ctx, 2*time.Millisecond, func(item []uint8) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.UInt8SliceView(ctx, 10*time.Millisecond, func(item []uint8) {
			inview <- item
		}, incoming)
...
func TestUInt8SliceFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt8SliceFilter(ctx, 2*time.Millisecond, func(item []uint8) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []uint8, 0)
		outgoing := services.UInt8SliceFilter(ctx, 10*time.Millisecond, func(item []uint8) bool {
			return true
		}, incoming)
...
func TestUInt8SliceCollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.UInt8SliceCollectUntil(ctx, 2*time.Millisecond, func(item [][]uint8) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan []uint8, 0)
		outgoing := services.UInt8SliceCollectUntil(ctx, 10*time.Millisecond, func(items [][]uint8) bool {
			return len(items) == 2
		}, incoming)
...
func TestUInt8SliceMergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt8SliceMergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UInt8SliceMergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt8SliceMergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt8SliceMergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt8SliceMergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt8SliceCombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt8SliceCombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.UInt8SliceCombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt8SliceCombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt8SliceCombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UInt8SliceCombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt8SliceCombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt8SliceCombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt8SliceCombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt8SliceCombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt8SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt8SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt8SliceDistributor(t *testing.T) {
	dist := services.NewUInt8SliceDistributor(0, 1*time.Second)
	dist.Start()
	...
	case initial := <-incoming:
		...
		if !isUInt8SliceEqualSlice(initial, <-incoming2) {
		...
		if !isUInt8SliceEqualSlice(initial, <-incoming3) {
...
func isUInt8SliceEqualSlice(item1 []uint8, item2 []uint8) bool {
...
func isUInt8SliceEqualDoubleSlice(item1 [][]uint8, item2 [][]uint8) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isUInt8SliceEqualSlice(item, item2[index]) {
			return false
		}
	}
