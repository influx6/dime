...
//go:generate moz generate-file -fromFile ./struct_service.go -toDir ./impl/structservice

// StructServiceByteAdapter defines a function that that will take a channel of bytes and return a channel of struct{}.
type StructServiceByteAdapter func(chan []byte) chan struct{}

// StructServiceToByteAdapter defines a function that that will take a channel of bytes and return a channel of struct{}.
type StructServiceToByteAdapter func(chan struct{}) chan []byte

// StructService defines a interface for underline systems which want to communicate like
// in a stream using channels to send/recieve struct{} values. It allows different services to create adapters to
...
type StructService interface {
	...
	Receive(string) (<-chan struct{}, error)
