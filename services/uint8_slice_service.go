...
// method.
func UInt8SliceSink(ctx CancelContext, waitTime time.Duration, in chan<- []uint8) chan<- []uint8 {
	res := make(chan []uint8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SliceSinkFilter(ctx CancelContext, waitTime time.Duration, filterFn func([]uint8) bool, in chan<- []uint8) chan<- []uint8 {
	res := make(chan []uint8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SliceSinkMutate(ctx CancelContext, waitTime time.Duration, mutateFn func([]uint8) []uint8, in chan<- []uint8) chan<- []uint8 {
	res := make(chan []uint8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SliceSinkView(ctx CancelContext, waitTime time.Duration, viewFn func([]uint8), in chan<- []uint8) chan<- []uint8 {
	res := make(chan []uint8, 0)
...
func (d *UInt8SliceDistributor) Start() {
	if atomic.LoadInt64(&d.running) != 0 {
		return
	}
