...
//go:generate moz generate-file -fromFile ./uint8_slice_service.go -toDir ./impl/uint8slice

// UInt8SliceDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type UInt8SliceDataWriterFunc func([]uint8) bool

// UInt8SliceFromByteAdapter defines a function that that will take a channel of bytes and return a channel of []uint8.
type UInt8SliceFromByteAdapter func(CancelContext, <-chan []byte) <-chan []uint8

// UInt8SliceToByteAdapter defines a function that that will take a channel of bytes and return a channel of []uint8.
type UInt8SliceToByteAdapter func(CancelContext, <-chan []uint8) <-chan []byte

// UInt8SlicePartialCollect defines a function which returns a channel where the items of the incoming channel
...
func UInt8SlicePartialCollect(ctx CancelContext, waitTime time.Duration, in <-chan []uint8) <-chan [][]uint8 {
...
// UInt8SliceCollect defines a function which returns a channel where the items of the incoming channel
...
func UInt8SliceCollect(ctx CancelContext, waitTime time.Duration, in <-chan []uint8) <-chan [][]uint8 {
	res := make(chan [][]uint8, 0)
...
// UInt8SliceMutate defines a function which returns a channel where the items of the incoming channel
...
// This function guarantees complete data.
func UInt8SliceMutate(ctx CancelContext, waitTime time.Duration, mutateFn func([]uint8) []uint8, in <-chan []uint8) <-chan []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceView defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func UInt8SliceView(ctx CancelContext, waitTime time.Duration, viewFn func([]uint8), in <-chan []uint8) <-chan []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func UInt8SliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []uint8) chan<- []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func UInt8SliceWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- []uint8) UInt8SliceDataWriterFunc {
...
// UInt8SliceWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func UInt8SliceWriterFuncTo(ctx CancelContext, in chan<- []uint8) UInt8SliceDataWriterFunc {
	return func(val []uint8) bool {
		select {
		case <-ctx.Done():
			return false
...
// UInt8SliceSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]uint8) bool, in chan<- []uint8) chan<- []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]uint8) []uint8, in chan<- []uint8) chan<- []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]uint8), in chan<- []uint8) chan<- []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceFilter defines a function which returns a channel where the items of the incoming channel
// are filtered based on a function, till the provided channel is closed.
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func UInt8SliceFilter(ctx CancelContext, waitTime time.Duration, filterFn func([]uint8) bool, in <-chan []uint8) <-chan []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceCollectUntil defines a function which returns a channel where the items of the incoming channel
...
func UInt8SliceCollectUntil(ctx CancelContext, waitTime time.Duration, condition func([][]uint8) bool, in <-chan []uint8) <-chan [][]uint8 {
	res := make(chan [][]uint8, 0)
...
// UInt8SliceMergeWithoutOrder merges the incoming data from the []uint8 into a single stream of []uint8,
...
func UInt8SliceMergeWithoutOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan []uint8) <-chan []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceMergeInOrder merges the incoming data from the []uint8 into a single stream of []uint8,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt8SliceMergeInOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan []uint8) <-chan []uint8 {
	res := make(chan []uint8, 0)
...
// UInt8SliceCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// UInt8SliceCombineParitallyWithoutOrder makes the following guarantees:
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt8SliceCombinePartiallyWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []uint8) <-chan [][]uint8 {
	res := make(chan [][]uint8, 0)
...
// UInt8SliceCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// UInt8SliceCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt8SliceCombineWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []uint8) <-chan [][]uint8 {
	res := make(chan [][]uint8, 0)
...
// UInt8SliceCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// UInt8SliceCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt8SliceCombineInPartialOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []uint8) <-chan [][]uint8 {
	res := make(chan [][]uint8, 0)
...
// UInt8SliceCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt8SliceCombineInOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []uint8) <-chan [][]uint8 {
	res := make(chan [][]uint8, 0)
...
// UInt8SliceDistributor delivers messages to subscription channels which it manages internal,
...
type UInt8SliceDistributor struct {
	running             int64
	messages            chan []uint8
	closer              chan struct{}
...
// NewUInt8SliceDistributor returns a new instance of a UInt8SliceDistributor.
func NewUInt8SliceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *UInt8SliceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &UInt8SliceDistributor{
...
func (d *UInt8SliceDistributor) PublishDeadline(message []uint8, dur time.Duration) {
...
func (d *UInt8SliceDistributor) Publish(message []uint8) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *UInt8SliceDistributor) Subscribe(sub chan<- []uint8) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *UInt8SliceDistributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *UInt8SliceDistributor) Stop() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *UInt8SliceDistributor) manage() {
...
// MonoUInt8SliceService defines a interface for underline systems which want to communicate like
...
// @iface
type MonoUInt8SliceService interface {
