...
func TestUIntSliceCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []uint, 0)
		outgoing := services.UIntSliceCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan []uint, 0)
		outgoing := services.UIntSliceCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUIntSlicePartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []uint, 0)
		outgoing := services.UIntSlicePartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan []uint, 0)
		outgoing := services.UIntSlicePartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUIntSliceMutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.UIntSliceMutate(ctx, 2*time.Millisecond, func(item []uint) []uint { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan []uint, 0)
		outgoing := services.UIntSliceMutate(ctx, 10*time.Millisecond, func(item []uint) []uint {
			if len(item) > 2 {
				return item[0:2]
			} else {
				return item
...
func TestUIntSliceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.UIntSliceView(ctx, 2*time.Millisecond, func(item []uint) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []uint, 0)
		inview := make(chan []uint, 0)

		outgoing := services.UIntSliceView(ctx, 10*time.Millisecond, func(item []uint) {
			inview <- item
		}, incoming)
...
func TestUIntSliceFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.UIntSliceFilter(ctx, 2*time.Millisecond, func(item []uint) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []uint, 0)
		outgoing := services.UIntSliceFilter(ctx, 10*time.Millisecond, func(item []uint) bool {
			return true
		}, incoming)
...
func TestUIntSliceCollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.UIntSliceCollectUntil(ctx, 2*time.Millisecond, func(item [][]uint) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan []uint, 0)
		outgoing := services.UIntSliceCollectUntil(ctx, 10*time.Millisecond, func(items [][]uint) bool {
			return len(items) == 2
		}, incoming)
...
func TestUIntSliceMergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntSliceMergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		incoming2 := make(chan []uint, 0)
		defer close(incoming2)

		incoming3 := make(chan []uint, 0)
		defer close(incoming3)

		outgoing := services.UIntSliceMergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntSliceMergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntSliceMergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UIntSliceMergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntSliceCombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntSliceCombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.UIntSliceCombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntSliceCombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntSliceCombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UIntSliceCombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntSliceCombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntSliceCombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UIntSliceCombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntSliceCombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntSliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.UIntSliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntSliceDistributor(t *testing.T) {
	...
	case initial := <-incoming:
		...
		if !isUIntSliceEqualSlice(initial, <-incoming2) {
		...
		if !isUIntSliceEqualSlice(initial, <-incoming3) {
...
func isUIntSliceEqualSlice(item1 []uint, item2 []uint) bool {
...
func isUIntSliceEqualDoubleSlice(item1 [][]uint, item2 [][]uint) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isUIntSliceEqualSlice(item, item2[index]) {
			return false
		}
	}
