...
func TestUIntCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.UIntCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan uint, 0)
		outgoing := services.UIntCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUIntPartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan uint, 0)
		outgoing := services.UIntPartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan uint, 0)
		outgoing := services.UIntPartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUIntMutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.UIntMutate(ctx, 2*time.Millisecond, func(item uint) uint { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan uint, 0)
		outgoing := services.UIntMutate(ctx, 10*time.Millisecond, func(item uint) uint {
			return item
		}, incoming)
...
func TestUIntView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.UIntView(ctx, 2*time.Millisecond, func(item uint) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.UIntView(ctx, 10*time.Millisecond, func(item uint) {
			inview <- item
		}, incoming)
...
func TestUIntFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.UIntFilter(ctx, 2*time.Millisecond, func(item uint) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan uint, 0)
		outgoing := services.UIntFilter(ctx, 10*time.Millisecond, func(item uint) bool {
			return true
		}, incoming)
...
func TestUIntCollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.UIntCollectUntil(ctx, 2*time.Millisecond, func(item []uint) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan uint, 0)
		outgoing := services.UIntCollectUntil(ctx, 10*time.Millisecond, func(items []uint) bool {
			return len(items) == 2
		}, incoming)
...
func TestUIntMergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntMergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UIntMergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntMergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntMergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UIntMergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntCombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntCombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.UIntCombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntCombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntCombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UIntCombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntCombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntCombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UIntCombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntCombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UIntCombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.UIntCombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUIntDistributor(t *testing.T) {
	dist := services.NewUIntDistributor(0, 1*time.Second)
	dist.Start()
	...
	case initial := <-incoming:
		...
		if !isUIntEqual(initial, <-incoming2) {
		...
		if !isUIntEqual(initial, <-incoming3) {
...
func isUIntEqual(item1, item2 uint) bool {
...
func isUIntEqualSlice(item1 []uint, item2 []uint) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isUIntEqual(item, item2[index]) {
			return false
		}
	}
