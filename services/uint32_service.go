...
// UInt32Collect defines a function which returns a channel where the items of the incoming channel
...
// UInt32Mutate defines a function which returns a channel where the items of the incoming channel
...
// method.
func UInt32Sink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- uint32) chan<- uint32 {
	res := make(chan uint32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt32SinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(uint32) bool, in chan<- uint32) chan<- uint32 {
	res := make(chan uint32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt32SinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(uint32) uint32, in chan<- uint32) chan<- uint32 {
	res := make(chan uint32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt32SinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(uint32), in chan<- uint32) chan<- uint32 {
	res := make(chan uint32, 0)
...
// UInt32Filter defines a function which returns a channel where the items of the incoming channel
...
// UInt32CollectUntil defines a function which returns a channel where the items of the incoming channel
...
// UInt32MergeWithoutOrder merges the incoming data from the uint32 into a single stream of uint32,
...
// UInt32MergeInOrder merges the incoming data from the uint32 into a single stream of uint32,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// UInt32CombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// UInt32CombineParitallyWithoutOrder makes the following guarantees:
...
// UInt32CombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// UInt32CombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// UInt32CombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// UInt32CombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// UInt32CombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// UInt32Distributor delivers messages to subscription channels which it manages internal,
...
// NewUInt32Distributor returns a new instance of a UInt32Distributor.
func NewUInt32Distributor(buffer int, sendWaitBeforeAbort time.Duration) *UInt32Distributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &UInt32Distributor{
...
func (d *UInt32Distributor) PublishDeadline(message uint32, dur time.Duration) {
...
func (d *UInt32Distributor) Publish(message uint32) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *UInt32Distributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *UInt32Distributor) manage() {
...
// MonoUInt32Service defines a interface for underline systems which want to communicate like
...
// @iface
type MonoUInt32Service interface {
...
// UInt32Service defines a interface for underline systems which want to communicate like
// in a stream using channels to send/recieve uint32 values. It allows different services to create adapters to
// transform data coming in and out from the Service.
// Auto-Generated using the moz code-generator https://github.com/influx6/moz.
// @iface
type UInt32Service interface {
	// ReadErrors will return a channel which will allow reading errors from the Service until it it is closed.
	ReadErrors() <-chan error
