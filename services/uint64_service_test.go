...
func TestUInt64Collect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.UInt64Collect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan uint64, 0)
		outgoing := services.UInt64Collect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUInt64PartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan uint64, 0)
		outgoing := services.UInt64PartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan uint64, 0)
		outgoing := services.UInt64PartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestUInt64Mutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt64Mutate(ctx, 2*time.Millisecond, func(item uint64) uint64 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan uint64, 0)
		outgoing := services.UInt64Mutate(ctx, 10*time.Millisecond, func(item uint64) uint64 {
			return item
		}, incoming)
...
func TestUInt64View(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt64View(ctx, 2*time.Millisecond, func(item uint64) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.UInt64View(ctx, 10*time.Millisecond, func(item uint64) {
			inview <- item
		}, incoming)
...
func TestUInt64Filter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt64Filter(ctx, 2*time.Millisecond, func(item uint64) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan uint64, 0)
		outgoing := services.UInt64Filter(ctx, 10*time.Millisecond, func(item uint64) bool {
			return true
		}, incoming)
...
func TestUInt64CollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.UInt64CollectUntil(ctx, 2*time.Millisecond, func(item []uint64) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan uint64, 0)
		outgoing := services.UInt64CollectUntil(ctx, 10*time.Millisecond, func(items []uint64) bool {
			return len(items) == 2
		}, incoming)
...
func TestUInt64MergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt64MergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UInt64MergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt64MergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt64MergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt64MergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt64CombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt64CombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.UInt64CombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt64CombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt64CombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.UInt64CombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt64CombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt64CombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt64CombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt64CombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.UInt64CombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.UInt64CombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestUInt64Distributor(t *testing.T) {
	dist := services.NewUInt64Distributor(0, 1*time.Second)
	dist.Start()
	...
	case initial := <-incoming:
		...
		if !isUInt64Equal(initial, <-incoming2) {
		...
		if !isUInt64Equal(initial, <-incoming3) {
...
func isUInt64Equal(item1, item2 uint64) bool {
...
func isUInt64EqualSlice(item1 []uint64, item2 []uint64) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isUInt64Equal(item, item2[index]) {
			return false
		}
	}
