...
// UInt32SliceDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type UInt32SliceDataWriterFunc func([]uint32) bool
...
// UInt32SliceSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func UInt32SliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []uint32) chan<- []uint32 {
	res := make(chan []uint32, 0)
...
// UInt32SliceWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func UInt32SliceWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- []uint32) UInt32SliceDataWriterFunc {
...
// UInt32SliceWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func UInt32SliceWriterFuncTo(ctx CancelContext, in chan<- []uint32) UInt32SliceDataWriterFunc {
	return func(val []uint32) bool {
		select {
		case <-ctx.Done():
			return false
...
// UInt32SliceSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt32SliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]uint32) bool, in chan<- []uint32) chan<- []uint32 {
	res := make(chan []uint32, 0)
...
// UInt32SliceSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt32SliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]uint32) []uint32, in chan<- []uint32) chan<- []uint32 {
	res := make(chan []uint32, 0)
...
// UInt32SliceSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt32SliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]uint32), in chan<- []uint32) chan<- []uint32 {
	res := make(chan []uint32, 0)
...
func UInt32SliceMergeWithoutOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan []uint32) <-chan []uint32 {
	...
	go func() {
		...
		filledContent := make(map[int][]uint32, 0)
...
func NewUInt32SliceDistributor(buffer int) *UInt32SliceDistributor {
	dist := &UInt32SliceDistributor{
