...
// IntSliceDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type IntSliceDataWriterFunc func([]int) bool

// IntSliceFromByteAdapter defines a function that that will take a channel of bytes and return a channel of []int.
type IntSliceFromByteAdapter func(CancelContext, <-chan []byte) <-chan []int
...
// IntSliceCollect defines a function which returns a channel where the items of the incoming channel
...
// IntSliceMutate defines a function which returns a channel where the items of the incoming channel
...
// IntSliceView defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func IntSliceView(ctx CancelContext, waitTime time.Duration, viewFn func([]int), in <-chan []int) <-chan []int {
	res := make(chan []int, 0)
...
// IntSliceSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func IntSliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []int) chan<- []int {
	res := make(chan []int, 0)
...
// IntSliceWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func IntSliceWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- []int) IntSliceDataWriterFunc {
...
// IntSliceWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func IntSliceWriterFuncTo(ctx CancelContext, in chan<- []int) IntSliceDataWriterFunc {
	return func(val []int) bool {
		select {
		case <-ctx.Done():
			return false
...
// IntSliceSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func IntSliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]int) bool, in chan<- []int) chan<- []int {
	res := make(chan []int, 0)
...
// IntSliceSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func IntSliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]int) []int, in chan<- []int) chan<- []int {
	res := make(chan []int, 0)
...
// IntSliceSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func IntSliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]int), in chan<- []int) chan<- []int {
	res := make(chan []int, 0)
...
// IntSliceFilter defines a function which returns a channel where the items of the incoming channel
...
// IntSliceCollectUntil defines a function which returns a channel where the items of the incoming channel
...
// IntSliceMergeWithoutOrder merges the incoming data from the []int into a single stream of []int,
...
// IntSliceMergeInOrder merges the incoming data from the []int into a single stream of []int,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// IntSliceCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// IntSliceCombineParitallyWithoutOrder makes the following guarantees:
...
// IntSliceCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// IntSliceCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// IntSliceCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// IntSliceCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// IntSliceCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// IntSliceDistributor delivers messages to subscription channels which it manages internal,
...
type IntSliceDistributor struct {
	running             int64
	messages            chan []int
	closer              chan struct{}
...
// NewIntSliceDistributor returns a new instance of a IntSliceDistributor.
func NewIntSliceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *IntSliceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &IntSliceDistributor{
...
func (d *IntSliceDistributor) PublishDeadline(message []int, dur time.Duration) {
...
func (d *IntSliceDistributor) Publish(message []int) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *IntSliceDistributor) Subscribe(sub chan<- []int) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *IntSliceDistributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *IntSliceDistributor) Stop() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *IntSliceDistributor) manage() {
...
// MonoIntSliceService defines a interface for underline systems which want to communicate like
...
// @iface
type MonoIntSliceService interface {
