...
func MapMergeWithoutOrder(ctx context.Context, maxWaitTime time.Duration, senders ...chan map[string]string) chan []map[string]string {
	...
	go func() {
		...
		total := len(senders)
		filled := make(map[int]map[string]string, 0)
...
type MonoMapService interface {
	...
	Send() (<-chan map[string]string, error)
	...
	Receive(<-chan map[string]string) error
...
type MapService interface {
	// Send will return a channel which will allow reading from the Service it till it is closed.
	Send(string) (<-chan map[string]string, error)

	// Receive will take the channel, which will be written into the Service for it's internal processing
	// and the Service will continue to read form the channel till the channel is closed.
	// Useful for collating/collecting services.
	Receive(string, <-chan map[string]string) error
