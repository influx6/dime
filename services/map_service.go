...
// MapDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type MapDataWriterFunc func(map[string]string) bool

// MapFromByteAdapter defines a function that that will take a channel of bytes and return a channel of map[string]string.
type MapFromByteAdapter func(CancelContext, <-chan []byte) <-chan map[string]string
...
// MapCollect defines a function which returns a channel where the items of the incoming channel
...
// MapMutate defines a function which returns a channel where the items of the incoming channel
...
// MapView defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func MapView(ctx CancelContext, waitTime time.Duration, viewFn func(map[string]string), in <-chan map[string]string) <-chan map[string]string {
	res := make(chan map[string]string, 0)
...
// MapSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func MapSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- map[string]string) chan<- map[string]string {
	res := make(chan map[string]string, 0)
...
// MapWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func MapWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- map[string]string) MapDataWriterFunc {
...
// MapWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func MapWriterFuncTo(ctx CancelContext, in chan<- map[string]string) MapDataWriterFunc {
	return func(val map[string]string) bool {
		select {
		case <-ctx.Done():
			return false
...
// MapSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func MapSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(map[string]string) bool, in chan<- map[string]string) chan<- map[string]string {
	res := make(chan map[string]string, 0)
...
// MapSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func MapSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(map[string]string) map[string]string, in chan<- map[string]string) chan<- map[string]string {
	res := make(chan map[string]string, 0)
...
// MapSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func MapSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(map[string]string), in chan<- map[string]string) chan<- map[string]string {
	res := make(chan map[string]string, 0)
...
// MapFilter defines a function which returns a channel where the items of the incoming channel
...
// MapCollectUntil defines a function which returns a channel where the items of the incoming channel
...
// MapMergeWithoutOrder merges the incoming data from the map[string]string into a single stream of map[string]string,
...
// MapMergeInOrder merges the incoming data from the map[string]string into a single stream of map[string]string,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// MapCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// MapCombineParitallyWithoutOrder makes the following guarantees:
...
// MapCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// MapCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// MapCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// MapCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// MapCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// MapDistributor delivers messages to subscription channels which it manages internal,
...
// NewMapDistributor returns a new instance of a MapDistributor.
func NewMapDistributor(buffer int, sendWaitBeforeAbort time.Duration) *MapDistributor {
...
func (d *MapDistributor) PublishDeadline(message map[string]string, dur time.Duration) {
...
func (d *MapDistributor) Publish(message map[string]string) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *MapDistributor) Subscribe(sub chan<- map[string]string) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// MonoMapService defines a interface for underline systems which want to communicate like
...
// MapService defines a interface for underline systems which want to communicate like
