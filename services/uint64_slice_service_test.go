...
func TestUInt64SliceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.UInt64SliceView(ctx, 2*time.Millisecond, func(item []uint64) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []uint64, 0)
		inview := make(chan []uint64, 0)

		outgoing := services.UInt64SliceView(ctx, 10*time.Millisecond, func(item []uint64) {
			inview <- item
		}, incoming)
...
func TestUInt64SliceDistributor(t *testing.T) {
	dist := services.NewUInt64SliceDistributor(0, 1*time.Second)
	dist.Start()
