...
func TestFloat64Collect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.Float64Collect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan float64, 0)
		outgoing := services.Float64Collect(context.Background(), 10*time.Millisecond, incoming)
...
func TestFloat64PartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan float64, 0)
		outgoing := services.Float64PartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan float64, 0)
		outgoing := services.Float64PartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestFloat64Mutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.Float64Mutate(ctx, 2*time.Millisecond, func(item float64) float64 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan float64, 0)
		outgoing := services.Float64Mutate(ctx, 10*time.Millisecond, func(item float64) float64 {
			return item
		}, incoming)
...
func TestFloat64View(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.Float64View(ctx, 2*time.Millisecond, func(item float64) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan float64, 0)
		inview := make(chan float64, 0)

		outgoing := services.Float64View(ctx, 10*time.Millisecond, func(item float64) {
			inview <- item
		}, incoming)
...
func TestFloat64Filter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.Float64Filter(ctx, 2*time.Millisecond, func(item float64) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan float64, 0)
		outgoing := services.Float64Filter(ctx, 10*time.Millisecond, func(item float64) bool {
			return true
		}, incoming)
...
func TestFloat64CollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.Float64CollectUntil(ctx, 2*time.Millisecond, func(item []float64) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan float64, 0)
		outgoing := services.Float64CollectUntil(ctx, 10*time.Millisecond, func(items []float64) bool {
			return len(items) == 2
		}, incoming)
...
func TestFloat64MergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Float64MergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Float64MergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestFloat64MergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Float64MergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Float64MergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestFloat64CombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Float64CombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.Float64CombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestFloat64CombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Float64CombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Float64CombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestFloat64CombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Float64CombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Float64CombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestFloat64CombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Float64CombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.Float64CombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestFloat64Distributor(t *testing.T) {
	dist := services.NewFloat64Distributor(0, 1*time.Second)
	dist.Start()
	...
	case initial := <-incoming:
		...
		if !isFloat64Equal(initial, <-incoming2) {
		...
		if !isFloat64Equal(initial, <-incoming3) {
...
func isFloat64EqualSlice(item1 []float64, item2 []float64) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isFloat64Equal(item, item2[index]) {
			return false
		}
	}
