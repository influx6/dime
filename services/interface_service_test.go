...
func TestInterfaceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.InterfaceView(ctx, 2*time.Millisecond, func(item interface{}) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.InterfaceView(ctx, 10*time.Millisecond, func(item interface{}) {
			inview <- item
		}, incoming)
