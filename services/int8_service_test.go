...
func TestInt8Collect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.Int8Collect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan int8, 0)
		outgoing := services.Int8Collect(context.Background(), 10*time.Millisecond, incoming)
...
func TestInt8PartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan int8, 0)
		outgoing := services.Int8PartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan int8, 0)
		outgoing := services.Int8PartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestInt8Mutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int8Mutate(ctx, 2*time.Millisecond, func(item int8) int8 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan int8, 0)
		outgoing := services.Int8Mutate(ctx, 10*time.Millisecond, func(item int8) int8 {
			return item
		}, incoming)
...
func TestInt8View(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int8View(ctx, 2*time.Millisecond, func(item int8) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.Int8View(ctx, 10*time.Millisecond, func(item int8) {
			inview <- item
		}, incoming)
...
func TestInt8Filter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int8Filter(ctx, 2*time.Millisecond, func(item int8) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan int8, 0)
		outgoing := services.Int8Filter(ctx, 10*time.Millisecond, func(item int8) bool {
			return true
		}, incoming)
...
func TestInt8CollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.Int8CollectUntil(ctx, 2*time.Millisecond, func(item []int8) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan int8, 0)
		outgoing := services.Int8CollectUntil(ctx, 10*time.Millisecond, func(items []int8) bool {
			return len(items) == 2
		}, incoming)
...
func TestInt8MergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Int8MergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Int8MergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt8MergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int8MergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int8MergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt8CombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int8CombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.Int8CombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt8CombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Int8CombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Int8CombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt8CombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int8CombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int8CombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt8CombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int8CombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int8CombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt8Distributor(t *testing.T) {
	dist := services.NewInt8Distributor(0, 1*time.Second)
	...
	case initial := <-incoming:
		...
		if !isInt8Equal(initial, <-incoming2) {
		...
		if !isInt8Equal(initial, <-incoming3) {
...
func isInt8Equal(item1, item2 int8) bool {
...
func isInt8EqualSlice(item1 []int8, item2 []int8) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isInt8Equal(item, item2[index]) {
			return false
		}
	}
