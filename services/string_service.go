...
// StringCollect defines a function which returns a channel where the items of the incoming channel
...
// StringMutate defines a function which returns a channel where the items of the incoming channel
...
// method.
func StringSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- string) chan<- string {
	res := make(chan string, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func StringSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(string) bool, in chan<- string) chan<- string {
	res := make(chan string, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func StringSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(string) string, in chan<- string) chan<- string {
	res := make(chan string, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func StringSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(string), in chan<- string) chan<- string {
	res := make(chan string, 0)
...
// StringFilter defines a function which returns a channel where the items of the incoming channel
...
// StringCollectUntil defines a function which returns a channel where the items of the incoming channel
...
// StringMergeWithoutOrder merges the incoming data from the string into a single stream of string,
...
// StringMergeInOrder merges the incoming data from the string into a single stream of string,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// StringCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// StringCombineParitallyWithoutOrder makes the following guarantees:
...
// StringCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// StringCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// StringCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// StringCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// StringCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// StringDistributor delivers messages to subscription channels which it manages internal,
...
// NewStringDistributor returns a new instance of a StringDistributor.
func NewStringDistributor(buffer int, sendWaitBeforeAbort time.Duration) *StringDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &StringDistributor{
...
func (d *StringDistributor) PublishDeadline(message string, dur time.Duration) {
...
func (d *StringDistributor) Publish(message string) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *StringDistributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *StringDistributor) manage() {
...
// MonoStringService defines a interface for underline systems which want to communicate like
...
// @iface
type MonoStringService interface {
...
// StringService defines a interface for underline systems which want to communicate like
