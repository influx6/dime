...
func TestComplex128Collect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.Complex128Collect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan complex128, 0)
		outgoing := services.Complex128Collect(context.Background(), 10*time.Millisecond, incoming)
...
func TestComplex128PartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan complex128, 0)
		outgoing := services.Complex128PartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan complex128, 0)
		outgoing := services.Complex128PartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestComplex128Mutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.Complex128Mutate(ctx, 2*time.Millisecond, func(item complex128) complex128 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan complex128, 0)
		outgoing := services.Complex128Mutate(ctx, 10*time.Millisecond, func(item complex128) complex128 {
			return item
		}, incoming)
...
func TestComplex128View(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.Complex128View(ctx, 2*time.Millisecond, func(item complex128) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.Complex128View(ctx, 10*time.Millisecond, func(item complex128) {
			inview <- item
		}, incoming)
...
func TestComplex128Filter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.Complex128Filter(ctx, 2*time.Millisecond, func(item complex128) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan complex128, 0)
		outgoing := services.Complex128Filter(ctx, 10*time.Millisecond, func(item complex128) bool {
			return true
		}, incoming)
...
func TestComplex128CollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.Complex128CollectUntil(ctx, 2*time.Millisecond, func(item []complex128) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan complex128, 0)
		outgoing := services.Complex128CollectUntil(ctx, 10*time.Millisecond, func(items []complex128) bool {
			return len(items) == 2
		}, incoming)
...
func TestComplex128MergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128MergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Complex128MergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128MergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128MergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex128MergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128CombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128CombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.Complex128CombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128CombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128CombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Complex128CombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128CombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128CombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex128CombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128CombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128CombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex128CombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128Distributor(t *testing.T) {
	dist := services.NewComplex128Distributor(0, 1*time.Second)
	...
	case initial := <-incoming:
		...
		if !isComplex128Equal(initial, <-incoming2) {
		...
		if !isComplex128Equal(initial, <-incoming3) {
...
func isComplex128Equal(item1, item2 complex128) bool {
...
func isComplex128EqualSlice(item1 []complex128, item2 []complex128) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isComplex128Equal(item, item2[index]) {
			return false
		}
	}
