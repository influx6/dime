...
// method.
func Float32Sink(ctx CancelContext, waitTime time.Duration, in chan<- float32) chan<- float32 {
	res := make(chan float32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SinkFilter(ctx CancelContext, waitTime time.Duration, filterFn func(float32) bool, in chan<- float32) chan<- float32 {
	res := make(chan float32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SinkMutate(ctx CancelContext, waitTime time.Duration, mutateFn func(float32) float32, in chan<- float32) chan<- float32 {
	res := make(chan float32, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SinkView(ctx CancelContext, waitTime time.Duration, viewFn func(float32), in chan<- float32) chan<- float32 {
	res := make(chan float32, 0)
...
func (d *Float32Distributor) Start() {
	if atomic.LoadInt64(&d.running) != 0 {
		return
	}
