...
//go:generate moz generate-file -fromFile ./float32_service.go -toDir ./impl/float32

// Float32DataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type Float32DataWriterFunc func(float32) bool

// Float32FromByteAdapter defines a function that that will take a channel of bytes and return a channel of float32.
type Float32FromByteAdapter func(CancelContext, <-chan []byte) <-chan float32

// Float32ToByteAdapter defines a function that that will take a channel of bytes and return a channel of float32.
type Float32ToByteAdapter func(CancelContext, <-chan float32) <-chan []byte

// Float32PartialCollect defines a function which returns a channel where the items of the incoming channel
...
func Float32PartialCollect(ctx CancelContext, waitTime time.Duration, in <-chan float32) <-chan []float32 {
...
// Float32Collect defines a function which returns a channel where the items of the incoming channel
...
func Float32Collect(ctx CancelContext, waitTime time.Duration, in <-chan float32) <-chan []float32 {
	res := make(chan []float32, 0)
...
// Float32Mutate defines a function which returns a channel where the items of the incoming channel
...
// This function guarantees complete data.
func Float32Mutate(ctx CancelContext, waitTime time.Duration, mutateFn func(float32) float32, in <-chan float32) <-chan float32 {
	res := make(chan float32, 0)
...
// Float32View defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Float32View(ctx CancelContext, waitTime time.Duration, viewFn func(float32), in <-chan float32) <-chan float32 {
	res := make(chan float32, 0)
...
// Float32Sink defines a function which returns a channel, where the items of the returned channel
...
// method.
func Float32Sink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- float32) chan<- float32 {
	res := make(chan float32, 0)
...
// Float32WriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func Float32WriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- float32) Float32DataWriterFunc {
	return func(val float32) bool {
		select {
		case <-ctx.Done():
			return false
		case in <- val:
...
// Float32WriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func Float32WriterFuncTo(ctx CancelContext, in chan<- float32) Float32DataWriterFunc {
	return func(val float32) bool {
		select {
		case <-ctx.Done():
			return false
...
// Float32SinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(float32) bool, in chan<- float32) chan<- float32 {
	res := make(chan float32, 0)
...
// Float32SinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(float32) float32, in chan<- float32) chan<- float32 {
	res := make(chan float32, 0)
...
// Float32SinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(float32), in chan<- float32) chan<- float32 {
	res := make(chan float32, 0)
...
// Float32Filter defines a function which returns a channel where the items of the incoming channel
// are filtered based on a function, till the provided channel is closed.
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Float32Filter(ctx CancelContext, waitTime time.Duration, filterFn func(float32) bool, in <-chan float32) <-chan float32 {
	res := make(chan float32, 0)
...
// Float32CollectUntil defines a function which returns a channel where the items of the incoming channel
...
func Float32CollectUntil(ctx CancelContext, waitTime time.Duration, condition func([]float32) bool, in <-chan float32) <-chan []float32 {
	res := make(chan []float32, 0)
...
// Float32MergeWithoutOrder merges the incoming data from the float32 into a single stream of float32,
...
func Float32MergeWithoutOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan float32) <-chan []float32 {
	res := make(chan []float32, 0)
	...
	go func() {
		...
		filledContent := make(map[int]float32, 0)
...
// Float32MergeInOrder merges the incoming data from the float32 into a single stream of float32,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Float32MergeInOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan float32) <-chan []float32 {
	res := make(chan []float32, 0)
...
// Float32CombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// Float32CombineParitallyWithoutOrder makes the following guarantees:
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Float32CombinePartiallyWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan float32) <-chan []float32 {
	res := make(chan []float32, 0)
	...
	go func() {
		...
		for {
			...
			if len(content) == total {
				...
				index = 0
				filled = make(map[int]bool, 0)
				content = make([]float32, len(senders))
			}
...
// Float32CombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Float32CombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Float32CombineWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan float32) <-chan []float32 {
	res := make(chan []float32, 0)
...
// Float32CombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Float32CombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Float32CombineInPartialOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan float32) <-chan []float32 {
	res := make(chan []float32, 0)
...
// Float32CombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Float32CombineInOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan float32) <-chan []float32 {
	res := make(chan []float32, 0)
...
// Float32Distributor delivers messages to subscription channels which it manages internal,
...
type Float32Distributor struct {
	running             int64
	messages            chan float32
	closer              chan struct{}
	subcloser           chan struct{}
	clear               chan struct{}
	subscribers         []chan<- float32
	newSub              chan chan<- float32
	sendWaitBeforeAbort time.Duration
}

// NewFloat32Distributor returns a new instance of a Float32Distributor.
func NewFloat32Distributor(buffer int, sendWaitBeforeAbort time.Duration) *Float32Distributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &Float32Distributor{
		clear:               make(chan struct{}, 0),
		closer:              make(chan struct{}, 0),
		subcloser:           make(chan struct{}, 0),
		subscribers:         make([]chan<- float32, 0),
		newSub:              make(chan chan<- float32, 0),
		messages:            make(chan float32, buffer),
...
func (d *Float32Distributor) PublishDeadline(message float32, dur time.Duration) {
...
func (d *Float32Distributor) Publish(message float32) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Float32Distributor) Subscribe(sub chan<- float32) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Float32Distributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Float32Distributor) Stop() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *Float32Distributor) manage() {
...
// MonoFloat32Service defines a interface for underline systems which want to communicate like
// in a stream using channels to send/recieve only float32 values from a single source.
...
// @iface
type MonoFloat32Service interface {
...
// Float32Service defines a interface for underline systems which want to communicate like
// in a stream using channels to send/recieve float32 values. It allows different services to create adapters to
// transform data coming in and out from the Service.
// Auto-Generated using the moz code-generator https://github.com/influx6/moz.
// @iface
type Float32Service interface {
	// ReadErrors will return a channel which will allow reading errors from the Service until it it is closed.
	ReadErrors() <-chan error

	// Read will return a channel which will allow reading from the Service until it it is closed.
	Read(string) (<-chan float32, error)

	// Receive will take the channel, which will be writing into the Service for it's internal processing
	// and the Service will continue to read form the channel till the channel is closed.
	// Useful for collating/collecting services.
	Write(string, <-chan float32) error
