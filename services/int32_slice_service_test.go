...
func TestInt32SliceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int32SliceView(ctx, 2*time.Millisecond, func(item []int32) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.Int32SliceView(ctx, 10*time.Millisecond, func(item []int32) {
			inview <- item
		}, incoming)
...
func TestInt32SliceDistributor(t *testing.T) {
	dist := services.NewInt32SliceDistributor(0, 1*time.Second)
