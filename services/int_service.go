...
// method.
func IntSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- int) chan<- int {
	res := make(chan int, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func IntSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(int) bool, in chan<- int) chan<- int {
	res := make(chan int, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func IntSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(int) int, in chan<- int) chan<- int {
	res := make(chan int, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func IntSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(int), in chan<- int) chan<- int {
	res := make(chan int, 0)
...
func NewIntDistributor(buffer int) *IntDistributor {
	dist := &IntDistributor{
