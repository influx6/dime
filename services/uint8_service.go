...
// method.
func UInt8Sink(ctx CancelContext, waitTime time.Duration, in chan<- uint8) chan<- uint8 {
	res := make(chan uint8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SinkFilter(ctx CancelContext, waitTime time.Duration, filterFn func(uint8) bool, in chan<- uint8) chan<- uint8 {
	res := make(chan uint8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SinkMutate(ctx CancelContext, waitTime time.Duration, mutateFn func(uint8) uint8, in chan<- uint8) chan<- uint8 {
	res := make(chan uint8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt8SinkView(ctx CancelContext, waitTime time.Duration, viewFn func(uint8), in chan<- uint8) chan<- uint8 {
	res := make(chan uint8, 0)
...
func NewUInt8Distributor(buffer int, sendWaitBeforeAbort time.Duration) *UInt8Distributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	return &UInt8Distributor{
