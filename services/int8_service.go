...
// Int8ToByteAdapter defines a function that that will take a channel of bytes and return a channel of int8.
type Int8ToByteAdapter func(CancelContext, <-chan int8) <-chan []byte

// Int8PartialCollect defines a function which returns a channel where the items of the incoming channel
...
func Int8PartialCollect(ctx CancelContext, waitTime time.Duration, in <-chan int8) <-chan []int8 {
...
// Int8Collect defines a function which returns a channel where the items of the incoming channel
...
func Int8Collect(ctx CancelContext, waitTime time.Duration, in <-chan int8) <-chan []int8 {
	res := make(chan []int8, 0)
...
// Int8Mutate defines a function which returns a channel where the items of the incoming channel
...
// This function guarantees complete data.
func Int8Mutate(ctx CancelContext, waitTime time.Duration, mutateFn func(int8) int8, in <-chan int8) <-chan int8 {
	res := make(chan int8, 0)
...
// method.
func Int8Sink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- int8) chan<- int8 {
	res := make(chan int8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int8SinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(int8) bool, in chan<- int8) chan<- int8 {
	res := make(chan int8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int8SinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(int8) int8, in chan<- int8) chan<- int8 {
	res := make(chan int8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int8SinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(int8), in chan<- int8) chan<- int8 {
	res := make(chan int8, 0)
...
// Int8Filter defines a function which returns a channel where the items of the incoming channel
// are filtered based on a function, till the provided channel is closed.
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Int8Filter(ctx CancelContext, waitTime time.Duration, filterFn func(int8) bool, in <-chan int8) <-chan int8 {
	res := make(chan int8, 0)
...
// Int8CollectUntil defines a function which returns a channel where the items of the incoming channel
...
func Int8CollectUntil(ctx CancelContext, waitTime time.Duration, condition func([]int8) bool, in <-chan int8) <-chan []int8 {
	res := make(chan []int8, 0)
...
// Int8MergeWithoutOrder merges the incoming data from the int8 into a single stream of int8,
...
func Int8MergeWithoutOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan int8) <-chan []int8 {
	res := make(chan []int8, 0)
...
// Int8MergeInOrder merges the incoming data from the int8 into a single stream of int8,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int8MergeInOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan int8) <-chan []int8 {
	res := make(chan []int8, 0)
...
// Int8CombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// Int8CombineParitallyWithoutOrder makes the following guarantees:
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int8CombinePartiallyWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan int8) <-chan []int8 {
	res := make(chan []int8, 0)
...
// Int8CombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Int8CombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int8CombineWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan int8) <-chan []int8 {
	res := make(chan []int8, 0)
...
// Int8CombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Int8CombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int8CombineInPartialOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan int8) <-chan []int8 {
	res := make(chan []int8, 0)
...
// Int8CombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int8CombineInOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan int8) <-chan []int8 {
	res := make(chan []int8, 0)
...
// Int8Distributor delivers messages to subscription channels which it manages internal,
...
// NewInt8Distributor returns a new instance of a Int8Distributor.
func NewInt8Distributor(buffer int, sendWaitBeforeAbort time.Duration) *Int8Distributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &Int8Distributor{
...
func (d *Int8Distributor) PublishDeadline(message int8, dur time.Duration) {
...
func (d *Int8Distributor) Publish(message int8) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// MonoInt8Service defines a interface for underline systems which want to communicate like
...
// @iface
type MonoInt8Service interface {
