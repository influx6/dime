...
func TestComplex64SliceCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.Complex64SliceCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan []complex64, 0)
		outgoing := services.Complex64SliceCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestComplex64SlicePartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []complex64, 0)
		outgoing := services.Complex64SlicePartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan []complex64, 0)
		outgoing := services.Complex64SlicePartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestComplex64SliceMutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.Complex64SliceMutate(ctx, 2*time.Millisecond, func(item []complex64) []complex64 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan []complex64, 0)
		outgoing := services.Complex64SliceMutate(ctx, 10*time.Millisecond, func(item []complex64) []complex64 {
			if len(item) > 2 {
				return item[0:2]
			} else {
				return item
...
func TestComplex64SliceFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.Complex64SliceFilter(ctx, 2*time.Millisecond, func(item []complex64) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []complex64, 0)
		outgoing := services.Complex64SliceFilter(ctx, 10*time.Millisecond, func(item []complex64) bool {
			return true
		}, incoming)
...
func TestComplex64SliceCollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.Complex64SliceCollectUntil(ctx, 2*time.Millisecond, func(item [][]complex64) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan []complex64, 0)
		outgoing := services.Complex64SliceCollectUntil(ctx, 10*time.Millisecond, func(items [][]complex64) bool {
			return len(items) == 2
		}, incoming)
...
func TestComplex64SliceMergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex64SliceMergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Complex64SliceMergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
		...
		go func() {
			...
			incoming2 <- []complex64{1.0, 2.0, 3.0}
		...
		go func() {
			...
			incoming3 <- []complex64{1.0, 2.0, 3.0}
...
func TestComplex64SliceMergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex64SliceMergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex64SliceMergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex64SliceCombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex64SliceCombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.Complex64SliceCombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex64SliceCombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex64SliceCombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Complex64SliceCombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex64SliceCombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex64SliceCombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex64SliceCombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex64SliceCombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex64SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex64SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex64SliceDistributor(t *testing.T) {
	...
	case initial := <-incoming:
		...
		if !isComplex64SliceEqualSlice(initial, <-incoming2) {
		...
		if !isComplex64SliceEqualSlice(initial, <-incoming3) {
...
func isComplex64SliceEqualSlice(item1 []complex64, item2 []complex64) bool {
...
func isComplex64SliceEqualDoubleSlice(item1 [][]complex64, item2 [][]complex64) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isComplex64SliceEqualSlice(item, item2[index]) {
			return false
		}
	}
