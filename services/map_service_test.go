...
func TestMapView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.MapView(ctx, 2*time.Millisecond, func(item map[string]string) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.MapView(ctx, 10*time.Millisecond, func(item map[string]string) {
			inview <- item
		}, incoming)
...
func TestMapDistributor(t *testing.T) {
	dist := services.NewMapDistributor(0, 1*time.Second)
