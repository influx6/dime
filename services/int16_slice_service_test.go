...
func TestInt16SliceCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.Int16SliceCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan []int16, 0)
		outgoing := services.Int16SliceCollect(context.Background(), 10*time.Millisecond, incoming)

		go func() {
			for i := 3; i > 0; i-- {

				incoming <- []int16{1, 2, 3}
...
func TestInt16SlicePartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []int16, 0)
		outgoing := services.Int16SlicePartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan []int16, 0)
		outgoing := services.Int16SlicePartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestInt16SliceMutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int16SliceMutate(ctx, 2*time.Millisecond, func(item []int16) []int16 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan []int16, 0)
		outgoing := services.Int16SliceMutate(ctx, 10*time.Millisecond, func(item []int16) []int16 {
			if len(item) > 2 {
				return item[0:2]
			} else {
				return item
...
func TestInt16SliceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int16SliceView(ctx, 2*time.Millisecond, func(item []int16) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.Int16SliceView(ctx, 10*time.Millisecond, func(item []int16) {
			inview <- item
		}, incoming)
...
func TestInt16SliceFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int16SliceFilter(ctx, 2*time.Millisecond, func(item []int16) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []int16, 0)
		outgoing := services.Int16SliceFilter(ctx, 10*time.Millisecond, func(item []int16) bool {
			return true
		}, incoming)
...
func TestInt16SliceCollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.Int16SliceCollectUntil(ctx, 2*time.Millisecond, func(item [][]int16) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan []int16, 0)
		outgoing := services.Int16SliceCollectUntil(ctx, 10*time.Millisecond, func(items [][]int16) bool {
			return len(items) == 2
		}, incoming)
...
func TestInt16SliceMergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Int16SliceMergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Int16SliceMergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt16SliceMergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int16SliceMergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int16SliceMergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt16SliceCombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int16SliceCombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.Int16SliceCombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt16SliceCombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Int16SliceCombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Int16SliceCombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt16SliceCombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int16SliceCombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int16SliceCombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt16SliceCombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int16SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int16SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt16SliceDistributor(t *testing.T) {
	...
	case initial := <-incoming:
		...
		if !isInt16SliceEqualSlice(initial, <-incoming2) {
		...
		if !isInt16SliceEqualSlice(initial, <-incoming3) {
...
func isInt16SliceEqualSlice(item1 []int16, item2 []int16) bool {
...
func isInt16SliceEqualDoubleSlice(item1 [][]int16, item2 [][]int16) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isInt16SliceEqualSlice(item, item2[index]) {
			return false
		}
	}
