...
func TestIntView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.IntView(ctx, 2*time.Millisecond, func(item int) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.IntView(ctx, 10*time.Millisecond, func(item int) {
			inview <- item
		}, incoming)
...
func TestIntDistributor(t *testing.T) {
	dist := services.NewIntDistributor(0, 1*time.Second)
