...
// Complex64SliceDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type Complex64SliceDataWriterFunc func([]complex64) bool
...
// Complex64SliceSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func Complex64SliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []complex64) chan<- []complex64 {
	res := make(chan []complex64, 0)
...
// Complex64SliceWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func Complex64SliceWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- []complex64) Complex64SliceDataWriterFunc {
...
// Complex64SliceWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func Complex64SliceWriterFuncTo(ctx CancelContext, in chan<- []complex64) Complex64SliceDataWriterFunc {
	return func(val []complex64) bool {
		select {
		case <-ctx.Done():
			return false
...
// Complex64SliceSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex64SliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]complex64) bool, in chan<- []complex64) chan<- []complex64 {
	res := make(chan []complex64, 0)
...
// Complex64SliceSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex64SliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]complex64) []complex64, in chan<- []complex64) chan<- []complex64 {
	res := make(chan []complex64, 0)
...
// Complex64SliceSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex64SliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]complex64), in chan<- []complex64) chan<- []complex64 {
	res := make(chan []complex64, 0)
...
func NewComplex64SliceDistributor(buffer int) *Complex64SliceDistributor {
	dist := &Complex64SliceDistributor{
