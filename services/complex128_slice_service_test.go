...
func TestComplex128SliceCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []complex128, 0)
		outgoing := services.Complex128SliceCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan []complex128, 0)
		outgoing := services.Complex128SliceCollect(context.Background(), 10*time.Millisecond, incoming)

		go func() {
			for i := 3; i > 0; i-- {

				incoming <- []complex128{1.0, 2.0, 3.0}
...
func TestComplex128SlicePartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan []complex128, 0)
		outgoing := services.Complex128SlicePartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan []complex128, 0)
		outgoing := services.Complex128SlicePartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestComplex128SliceMutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.Complex128SliceMutate(ctx, 2*time.Millisecond, func(item []complex128) []complex128 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan []complex128, 0)
		outgoing := services.Complex128SliceMutate(ctx, 10*time.Millisecond, func(item []complex128) []complex128 {
			if len(item) > 2 {
				return item[0:2]
			} else {
				return item
...
func TestComplex128SliceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.Complex128SliceView(ctx, 2*time.Millisecond, func(item []complex128) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []complex128, 0)
		inview := make(chan []complex128, 0)

		outgoing := services.Complex128SliceView(ctx, 10*time.Millisecond, func(item []complex128) {
			inview <- item
		}, incoming)
...
func TestComplex128SliceFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.Complex128SliceFilter(ctx, 2*time.Millisecond, func(item []complex128) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan []complex128, 0)
		outgoing := services.Complex128SliceFilter(ctx, 10*time.Millisecond, func(item []complex128) bool {
			return true
		}, incoming)
...
func TestComplex128SliceCollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.Complex128SliceCollectUntil(ctx, 2*time.Millisecond, func(item [][]complex128) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan []complex128, 0)
		outgoing := services.Complex128SliceCollectUntil(ctx, 10*time.Millisecond, func(items [][]complex128) bool {
			return len(items) == 2
		}, incoming)
...
func TestComplex128SliceMergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128SliceMergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		incoming2 := make(chan []complex128, 0)
		defer close(incoming2)

		incoming3 := make(chan []complex128, 0)
		defer close(incoming3)

		outgoing := services.Complex128SliceMergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
		...
		go func() {
			...
			incoming2 <- []complex128{1.0, 2.0, 3.0}
		...
		go func() {
			...
			incoming3 <- []complex128{1.0, 2.0, 3.0}
...
func TestComplex128SliceMergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128SliceMergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex128SliceMergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128SliceCombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128SliceCombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.Complex128SliceCombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128SliceCombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128SliceCombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Complex128SliceCombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128SliceCombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128SliceCombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex128SliceCombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128SliceCombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Complex128SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.Complex128SliceCombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestComplex128SliceDistributor(t *testing.T) {
	dist := services.NewComplex128SliceDistributor(0, 1*time.Second)

	incoming := make(chan []complex128, 1)
	incoming2 := make(chan []complex128, 1)
	incoming3 := make(chan []complex128, 1)
	...
	case initial := <-incoming:
		...
		if !isComplex128SliceEqualSlice(initial, <-incoming2) {
		...
		if !isComplex128SliceEqualSlice(initial, <-incoming3) {
...
func isComplex128SliceEqualSlice(item1 []complex128, item2 []complex128) bool {
...
func isComplex128SliceEqualDoubleSlice(item1 [][]complex128, item2 [][]complex128) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isComplex128SliceEqualSlice(item, item2[index]) {
			return false
		}
	}
