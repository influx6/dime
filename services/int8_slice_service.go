...
// method.
func Int8SliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []int8) chan<- []int8 {
	res := make(chan []int8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int8SliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]int8) bool, in chan<- []int8) chan<- []int8 {
	res := make(chan []int8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int8SliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]int8) []int8, in chan<- []int8) chan<- []int8 {
	res := make(chan []int8, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int8SliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]int8), in chan<- []int8) chan<- []int8 {
	res := make(chan []int8, 0)
...
func NewInt8SliceDistributor(buffer int) *Int8SliceDistributor {
	dist := &Int8SliceDistributor{
