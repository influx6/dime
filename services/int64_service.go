...
// method.
func Int64Sink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- int64) chan<- int64 {
	res := make(chan int64, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int64SinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(int64) bool, in chan<- int64) chan<- int64 {
	res := make(chan int64, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int64SinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(int64) int64, in chan<- int64) chan<- int64 {
	res := make(chan int64, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int64SinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(int64), in chan<- int64) chan<- int64 {
	res := make(chan int64, 0)
...
func NewInt64Distributor(buffer int) *Int64Distributor {
	dist := &Int64Distributor{
