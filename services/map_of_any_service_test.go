...
func TestMapOfAnyCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.MapOfAnyCollect(ctx, 10*time.Millisecond, incoming)
		...
		go func() {
			...
			for i := 20; i > 0; i-- {
				...
				case incoming <- map[interface{}]interface{}{"day": "monday"}:
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan map[interface{}]interface{}, 0)
		outgoing := services.MapOfAnyCollect(context.Background(), 10*time.Millisecond, incoming)

		go func() {
			for i := 3; i > 0; i-- {

				incoming <- map[interface{}]interface{}{"day": "monday"}
...
func TestMapOfAnyPartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan map[interface{}]interface{}, 0)
		outgoing := services.MapOfAnyPartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan map[interface{}]interface{}, 0)
		outgoing := services.MapOfAnyPartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestMapOfAnyMutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.MapOfAnyMutate(ctx, 2*time.Millisecond, func(item map[interface{}]interface{}) map[interface{}]interface{} { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan map[interface{}]interface{}, 0)
		outgoing := services.MapOfAnyMutate(ctx, 10*time.Millisecond, func(item map[interface{}]interface{}) map[interface{}]interface{} {
			return item
		}, incoming)
...
func TestMapOfAnyView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.MapOfAnyView(ctx, 2*time.Millisecond, func(item map[interface{}]interface{}) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.MapOfAnyView(ctx, 10*time.Millisecond, func(item map[interface{}]interface{}) {
			inview <- item
		}, incoming)
...
func TestMapOfAnyFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.MapOfAnyFilter(ctx, 2*time.Millisecond, func(item map[interface{}]interface{}) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan map[interface{}]interface{}, 0)
		outgoing := services.MapOfAnyFilter(ctx, 10*time.Millisecond, func(item map[interface{}]interface{}) bool {
			return true
		}, incoming)
...
func TestMapOfAnyCollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.MapOfAnyCollectUntil(ctx, 2*time.Millisecond, func(item []map[interface{}]interface{}) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan map[interface{}]interface{}, 0)
		outgoing := services.MapOfAnyCollectUntil(ctx, 10*time.Millisecond, func(items []map[interface{}]interface{}) bool {
			return len(items) == 2
		}, incoming)
...
func TestMapOfAnyMergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.MapOfAnyMergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.MapOfAnyMergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
		...
		go func() {
			...
			incoming2 <- map[interface{}]interface{}{"day": "monday"}
		...
		go func() {
			...
			incoming3 <- map[interface{}]interface{}{"day": "monday"}
...
func TestMapOfAnyMergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.MapOfAnyMergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.MapOfAnyMergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestMapOfAnyCombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.MapOfAnyCombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.MapOfAnyCombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestMapOfAnyCombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.MapOfAnyCombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.MapOfAnyCombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestMapOfAnyCombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.MapOfAnyCombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.MapOfAnyCombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestMapOfAnyCombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.MapOfAnyCombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.MapOfAnyCombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestMapOfAnyDistributor(t *testing.T) {
	...
	dist.Publish(
		...
		map[interface{}]interface{}{"day": "monday"},
	)
	...
	case initial := <-incoming:
		...
		if !isMapOfAnyEqual(initial, <-incoming2) {
		...
		if !isMapOfAnyEqual(initial, <-incoming3) {
...
func isMapOfAnyEqual(item1, item2 map[interface{}]interface{}) bool {
...
func isMapOfAnyEqualSlice(item1 []map[interface{}]interface{}, item2 []map[interface{}]interface{}) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isMapOfAnyEqual(item, item2[index]) {
			return false
		}
	}
