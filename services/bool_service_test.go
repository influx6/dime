...
func TestBoolMutate(t *testing.T) {
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan bool, 0)
		outgoing := services.BoolMutate(ctx, 10*time.Millisecond, func(item bool) bool { return !item }, incoming)
		...
		received2 := <-outgoing
		if received2 != true {
			tests.Failed("Should have recieved true as value but got %t", received2)
		}
		tests.Passed("Should have recieved true as value")
...
func TestBoolFilter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.BoolFilter(ctx, 2*time.Millisecond, func(item bool) bool { return !item }, incoming)
