...
// Complex128DataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type Complex128DataWriterFunc func(complex128) bool

// Complex128FromByteAdapter defines a function that that will take a channel of bytes and return a channel of complex128.
type Complex128FromByteAdapter func(CancelContext, <-chan []byte) <-chan complex128
...
// Complex128View defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Complex128View(ctx CancelContext, waitTime time.Duration, viewFn func(complex128), in <-chan complex128) <-chan complex128 {
	res := make(chan complex128, 0)
...
// Complex128Sink defines a function which returns a channel, where the items of the returned channel
...
// method.
func Complex128Sink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- complex128) chan<- complex128 {
	res := make(chan complex128, 0)
...
// Complex128WriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func Complex128WriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- complex128) Complex128DataWriterFunc {
...
// Complex128WriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func Complex128WriterFuncTo(ctx CancelContext, in chan<- complex128) Complex128DataWriterFunc {
	return func(val complex128) bool {
		select {
		case <-ctx.Done():
			return false
...
// Complex128SinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex128SinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(complex128) bool, in chan<- complex128) chan<- complex128 {
	res := make(chan complex128, 0)
...
// Complex128SinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex128SinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(complex128) complex128, in chan<- complex128) chan<- complex128 {
	res := make(chan complex128, 0)
...
// Complex128SinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex128SinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(complex128), in chan<- complex128) chan<- complex128 {
	res := make(chan complex128, 0)
