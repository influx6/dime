...
func TestBoolSliceView(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.BoolSliceView(ctx, 2*time.Millisecond, func(item []bool) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.BoolSliceView(ctx, 10*time.Millisecond, func(item []bool) {
			inview <- item
		}, incoming)
