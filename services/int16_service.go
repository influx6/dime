...
//go:generate moz generate-file -fromFile ./int16_service.go -toDir ./impl/int16

// Int16DataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type Int16DataWriterFunc func(int16) bool

// Int16FromByteAdapter defines a function that that will take a channel of bytes and return a channel of int16.
type Int16FromByteAdapter func(CancelContext, <-chan []byte) <-chan int16

// Int16ToByteAdapter defines a function that that will take a channel of bytes and return a channel of int16.
type Int16ToByteAdapter func(CancelContext, <-chan int16) <-chan []byte

// Int16PartialCollect defines a function which returns a channel where the items of the incoming channel
...
func Int16PartialCollect(ctx CancelContext, waitTime time.Duration, in <-chan int16) <-chan []int16 {
...
// Int16Collect defines a function which returns a channel where the items of the incoming channel
...
func Int16Collect(ctx CancelContext, waitTime time.Duration, in <-chan int16) <-chan []int16 {
	res := make(chan []int16, 0)
...
// Int16Mutate defines a function which returns a channel where the items of the incoming channel
...
// This function guarantees complete data.
func Int16Mutate(ctx CancelContext, waitTime time.Duration, mutateFn func(int16) int16, in <-chan int16) <-chan int16 {
	res := make(chan int16, 0)
...
// Int16View defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Int16View(ctx CancelContext, waitTime time.Duration, viewFn func(int16), in <-chan int16) <-chan int16 {
	res := make(chan int16, 0)
...
// Int16Sink defines a function which returns a channel, where the items of the returned channel
...
// method.
func Int16Sink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- int16) chan<- int16 {
	res := make(chan int16, 0)
...
// Int16WriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func Int16WriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- int16) Int16DataWriterFunc {
...
// Int16WriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func Int16WriterFuncTo(ctx CancelContext, in chan<- int16) Int16DataWriterFunc {
	return func(val int16) bool {
		select {
		case <-ctx.Done():
			return false
...
// Int16SinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int16SinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(int16) bool, in chan<- int16) chan<- int16 {
	res := make(chan int16, 0)
...
// Int16SinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int16SinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(int16) int16, in chan<- int16) chan<- int16 {
	res := make(chan int16, 0)
...
// Int16SinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Int16SinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(int16), in chan<- int16) chan<- int16 {
	res := make(chan int16, 0)
...
// Int16Filter defines a function which returns a channel where the items of the incoming channel
// are filtered based on a function, till the provided channel is closed.
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Int16Filter(ctx CancelContext, waitTime time.Duration, filterFn func(int16) bool, in <-chan int16) <-chan int16 {
	res := make(chan int16, 0)
...
// Int16CollectUntil defines a function which returns a channel where the items of the incoming channel
...
func Int16CollectUntil(ctx CancelContext, waitTime time.Duration, condition func([]int16) bool, in <-chan int16) <-chan []int16 {
	res := make(chan []int16, 0)
...
// Int16MergeWithoutOrder merges the incoming data from the int16 into a single stream of int16,
...
func Int16MergeWithoutOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan int16) <-chan []int16 {
	res := make(chan []int16, 0)
...
// Int16MergeInOrder merges the incoming data from the int16 into a single stream of int16,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int16MergeInOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan int16) <-chan []int16 {
	res := make(chan []int16, 0)
...
// Int16CombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// Int16CombineParitallyWithoutOrder makes the following guarantees:
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int16CombinePartiallyWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan int16) <-chan []int16 {
	res := make(chan []int16, 0)
...
// Int16CombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Int16CombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int16CombineWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan int16) <-chan []int16 {
	res := make(chan []int16, 0)
...
// Int16CombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Int16CombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int16CombineInPartialOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan int16) <-chan []int16 {
	res := make(chan []int16, 0)
...
// Int16CombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Int16CombineInOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan int16) <-chan []int16 {
	res := make(chan []int16, 0)
...
// Int16Distributor delivers messages to subscription channels which it manages internal,
...
// NewInt16Distributor returns a new instance of a Int16Distributor.
func NewInt16Distributor(buffer int, sendWaitBeforeAbort time.Duration) *Int16Distributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &Int16Distributor{
...
func (d *Int16Distributor) PublishDeadline(message int16, dur time.Duration) {
...
func (d *Int16Distributor) Publish(message int16) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Int16Distributor) Subscribe(sub chan<- int16) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *Int16Distributor) manage() {
...
// MonoInt16Service defines a interface for underline systems which want to communicate like
...
// @iface
type MonoInt16Service interface {
...
// Int16Service defines a interface for underline systems which want to communicate like
// in a stream using channels to send/recieve int16 values. It allows different services to create adapters to
// transform data coming in and out from the Service.
// Auto-Generated using the moz code-generator https://github.com/influx6/moz.
// @iface
type Int16Service interface {
	// ReadErrors will return a channel which will allow reading errors from the Service until it it is closed.
	ReadErrors() <-chan error
