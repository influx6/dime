...
//go:generate moz generate-file -fromFile ./complex128_slice_service.go -toDir ./impl/complex128slice

// Complex128SliceDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type Complex128SliceDataWriterFunc func([]complex128) bool

// Complex128SliceFromByteAdapter defines a function that that will take a channel of bytes and return a channel of []complex128.
type Complex128SliceFromByteAdapter func(CancelContext, <-chan []byte) <-chan []complex128

// Complex128SliceToByteAdapter defines a function that that will take a channel of bytes and return a channel of []complex128.
type Complex128SliceToByteAdapter func(CancelContext, <-chan []complex128) <-chan []byte

// Complex128SlicePartialCollect defines a function which returns a channel where the items of the incoming channel
...
func Complex128SlicePartialCollect(ctx CancelContext, waitTime time.Duration, in <-chan []complex128) <-chan [][]complex128 {
...
// Complex128SliceCollect defines a function which returns a channel where the items of the incoming channel
...
func Complex128SliceCollect(ctx CancelContext, waitTime time.Duration, in <-chan []complex128) <-chan [][]complex128 {
	res := make(chan [][]complex128, 0)
...
// Complex128SliceMutate defines a function which returns a channel where the items of the incoming channel
...
// This function guarantees complete data.
func Complex128SliceMutate(ctx CancelContext, waitTime time.Duration, mutateFn func([]complex128) []complex128, in <-chan []complex128) <-chan []complex128 {
	res := make(chan []complex128, 0)
...
// Complex128SliceView defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Complex128SliceView(ctx CancelContext, waitTime time.Duration, viewFn func([]complex128), in <-chan []complex128) <-chan []complex128 {
	res := make(chan []complex128, 0)
...
// Complex128SliceSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func Complex128SliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []complex128) chan<- []complex128 {
	res := make(chan []complex128, 0)
...
// Complex128SliceWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func Complex128SliceWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- []complex128) Complex128SliceDataWriterFunc {
	return func(val []complex128) bool {
		select {
		case <-ctx.Done():
			return false
		case in <- val:
...
// Complex128SliceWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func Complex128SliceWriterFuncTo(ctx CancelContext, in chan<- []complex128) Complex128SliceDataWriterFunc {
	return func(val []complex128) bool {
		select {
		case <-ctx.Done():
			return false
...
// Complex128SliceSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex128SliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]complex128) bool, in chan<- []complex128) chan<- []complex128 {
	res := make(chan []complex128, 0)
...
// Complex128SliceSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex128SliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]complex128) []complex128, in chan<- []complex128) chan<- []complex128 {
	res := make(chan []complex128, 0)
...
// Complex128SliceSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex128SliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]complex128), in chan<- []complex128) chan<- []complex128 {
	res := make(chan []complex128, 0)
...
// Complex128SliceFilter defines a function which returns a channel where the items of the incoming channel
// are filtered based on a function, till the provided channel is closed.
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Complex128SliceFilter(ctx CancelContext, waitTime time.Duration, filterFn func([]complex128) bool, in <-chan []complex128) <-chan []complex128 {
	res := make(chan []complex128, 0)
...
// Complex128SliceCollectUntil defines a function which returns a channel where the items of the incoming channel
...
func Complex128SliceCollectUntil(ctx CancelContext, waitTime time.Duration, condition func([][]complex128) bool, in <-chan []complex128) <-chan [][]complex128 {
	res := make(chan [][]complex128, 0)
...
// Complex128SliceMergeWithoutOrder merges the incoming data from the []complex128 into a single stream of []complex128,
...
func Complex128SliceMergeWithoutOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan []complex128) <-chan []complex128 {
	res := make(chan []complex128, 0)
	...
	go func() {
		...
		filledContent := make(map[int][]complex128, 0)
...
// Complex128SliceMergeInOrder merges the incoming data from the []complex128 into a single stream of []complex128,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Complex128SliceMergeInOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan []complex128) <-chan []complex128 {
	res := make(chan []complex128, 0)
...
// Complex128SliceCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// Complex128SliceCombineParitallyWithoutOrder makes the following guarantees:
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Complex128SliceCombinePartiallyWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []complex128) <-chan [][]complex128 {
	res := make(chan [][]complex128, 0)
...
// Complex128SliceCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Complex128SliceCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Complex128SliceCombineWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []complex128) <-chan [][]complex128 {
	res := make(chan [][]complex128, 0)
...
// Complex128SliceCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Complex128SliceCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Complex128SliceCombineInPartialOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []complex128) <-chan [][]complex128 {
	res := make(chan [][]complex128, 0)
...
// Complex128SliceCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func Complex128SliceCombineInOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []complex128) <-chan [][]complex128 {
	res := make(chan [][]complex128, 0)
...
// Complex128SliceDistributor delivers messages to subscription channels which it manages internal,
...
type Complex128SliceDistributor struct {
	running             int64
...
// NewComplex128SliceDistributor returns a new instance of a Complex128SliceDistributor.
func NewComplex128SliceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *Complex128SliceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &Complex128SliceDistributor{
...
func (d *Complex128SliceDistributor) PublishDeadline(message []complex128, dur time.Duration) {
...
func (d *Complex128SliceDistributor) Publish(message []complex128) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Complex128SliceDistributor) Subscribe(sub chan<- []complex128) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Complex128SliceDistributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Complex128SliceDistributor) Stop() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *Complex128SliceDistributor) manage() {
...
// MonoComplex128SliceService defines a interface for underline systems which want to communicate like
...
// @iface
type MonoComplex128SliceService interface {
...
// Complex128SliceService defines a interface for underline systems which want to communicate like
// in a stream using channels to send/recieve []complex128 values. It allows different services to create adapters to
// transform data coming in and out from the Service.
// Auto-Generated using the moz code-generator https://github.com/influx6/moz.
// @iface
type Complex128SliceService interface {
	// ReadErrors will return a channel which will allow reading errors from the Service until it it is closed.
	ReadErrors() <-chan error
