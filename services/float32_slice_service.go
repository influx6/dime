...
// Float32SliceDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type Float32SliceDataWriterFunc func([]float32) bool

// Float32SliceFromByteAdapter defines a function that that will take a channel of bytes and return a channel of []float32.
type Float32SliceFromByteAdapter func(CancelContext, <-chan []byte) <-chan []float32
...
// Float32SliceCollect defines a function which returns a channel where the items of the incoming channel
...
// Float32SliceMutate defines a function which returns a channel where the items of the incoming channel
...
// Float32SliceView defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func Float32SliceView(ctx CancelContext, waitTime time.Duration, viewFn func([]float32), in <-chan []float32) <-chan []float32 {
	res := make(chan []float32, 0)
...
// Float32SliceSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func Float32SliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []float32) chan<- []float32 {
	res := make(chan []float32, 0)
...
// Float32SliceWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func Float32SliceWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- []float32) Float32SliceDataWriterFunc {
...
// Float32SliceWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func Float32SliceWriterFuncTo(ctx CancelContext, in chan<- []float32) Float32SliceDataWriterFunc {
	return func(val []float32) bool {
		select {
		case <-ctx.Done():
			return false
...
// Float32SliceSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]float32) bool, in chan<- []float32) chan<- []float32 {
	res := make(chan []float32, 0)
...
// Float32SliceSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]float32) []float32, in chan<- []float32) chan<- []float32 {
	res := make(chan []float32, 0)
...
// Float32SliceSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Float32SliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]float32), in chan<- []float32) chan<- []float32 {
	res := make(chan []float32, 0)
...
// Float32SliceFilter defines a function which returns a channel where the items of the incoming channel
...
// Float32SliceCollectUntil defines a function which returns a channel where the items of the incoming channel
...
// Float32SliceMergeWithoutOrder merges the incoming data from the []float32 into a single stream of []float32,
...
// Float32SliceMergeInOrder merges the incoming data from the []float32 into a single stream of []float32,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// Float32SliceCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// Float32SliceCombineParitallyWithoutOrder makes the following guarantees:
...
// Float32SliceCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Float32SliceCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// Float32SliceCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Float32SliceCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// Float32SliceCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// Float32SliceDistributor delivers messages to subscription channels which it manages internal,
...
type Float32SliceDistributor struct {
	running             int64
...
// NewFloat32SliceDistributor returns a new instance of a Float32SliceDistributor.
func NewFloat32SliceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *Float32SliceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &Float32SliceDistributor{
...
func (d *Float32SliceDistributor) PublishDeadline(message []float32, dur time.Duration) {
...
func (d *Float32SliceDistributor) Publish(message []float32) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Float32SliceDistributor) Subscribe(sub chan<- []float32) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Float32SliceDistributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Float32SliceDistributor) Stop() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *Float32SliceDistributor) manage() {
...
// MonoFloat32SliceService defines a interface for underline systems which want to communicate like
...
// @iface
type MonoFloat32SliceService interface {
