...
// Complex64DataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type Complex64DataWriterFunc func(complex64) bool
...
// Complex64Collect defines a function which returns a channel where the items of the incoming channel
...
// Complex64Mutate defines a function which returns a channel where the items of the incoming channel
...
// Complex64Sink defines a function which returns a channel, where the items of the returned channel
...
// method.
func Complex64Sink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- complex64) chan<- complex64 {
	res := make(chan complex64, 0)
...
// Complex64WriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func Complex64WriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- complex64) Complex64DataWriterFunc {
	return func(val complex64) bool {
		select {
		case <-ctx.Done():
			return false
		case in <- val:
...
// Complex64WriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func Complex64WriterFuncTo(ctx CancelContext, in chan<- complex64) Complex64DataWriterFunc {
	return func(val complex64) bool {
		select {
		case <-ctx.Done():
			return false
...
// Complex64SinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex64SinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(complex64) bool, in chan<- complex64) chan<- complex64 {
	res := make(chan complex64, 0)
...
// Complex64SinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex64SinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(complex64) complex64, in chan<- complex64) chan<- complex64 {
	res := make(chan complex64, 0)
...
// Complex64SinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func Complex64SinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(complex64), in chan<- complex64) chan<- complex64 {
	res := make(chan complex64, 0)
...
// Complex64CollectUntil defines a function which returns a channel where the items of the incoming channel
...
// Complex64MergeWithoutOrder merges the incoming data from the complex64 into a single stream of complex64,
...
// Complex64MergeInOrder merges the incoming data from the complex64 into a single stream of complex64,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// Complex64CombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// Complex64CombineParitallyWithoutOrder makes the following guarantees:
...
// Complex64CombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Complex64CombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// Complex64CombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// Complex64CombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// Complex64CombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// Complex64Distributor delivers messages to subscription channels which it manages internal,
...
type Complex64Distributor struct {
	running     int64
...
func NewComplex64Distributor(buffer int) *Complex64Distributor {
	dist := &Complex64Distributor{
...
func (d *Complex64Distributor) PublishDeadline(message complex64, dur time.Duration) {
...
func (d *Complex64Distributor) Publish(message complex64) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Complex64Distributor) Subscribe(sub chan<- complex64) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Complex64Distributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *Complex64Distributor) Stop() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *Complex64Distributor) manage() {
...
// MonoComplex64Service defines a interface for underline systems which want to communicate like
...
// @iface
type MonoComplex64Service interface {
...
// Complex64Service defines a interface for underline systems which want to communicate like
// in a stream using channels to send/recieve complex64 values. It allows different services to create adapters to
// transform data coming in and out from the Service.
// Auto-Generated using the moz code-generator https://github.com/influx6/moz.
// @iface
type Complex64Service interface {
	// ReadErrors will return a channel which will allow reading errors from the Service until it it is closed.
	ReadErrors() <-chan error
