...
func TestInt64Collect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		outgoing := services.Int64Collect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is not closed we should not get collected items")
	{
		...
		incoming := make(chan int64, 0)
		outgoing := services.Int64Collect(context.Background(), 10*time.Millisecond, incoming)
...
func TestInt64PartialCollect(t *testing.T) {
	t.Logf("When all data is received before 3 second")
	{
		...
		incoming := make(chan int64, 0)
		outgoing := services.Int64PartialCollect(ctx, 10*time.Millisecond, incoming)
	...
	t.Logf("When sending channel is closed we should still get collected items")
	{
		...
		incoming := make(chan int64, 0)
		outgoing := services.Int64PartialCollect(context.Background(), 10*time.Millisecond, incoming)
...
func TestInt64Mutate(t *testing.T) {
	t.Logf("When data is mutated but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int64Mutate(ctx, 2*time.Millisecond, func(item int64) int64 { return item }, incoming)
	...
	t.Logf("When data is mutated on each receive")
	{
		...
		incoming := make(chan int64, 0)
		outgoing := services.Int64Mutate(ctx, 10*time.Millisecond, func(item int64) int64 {
			return item
		}, incoming)
...
func TestInt64View(t *testing.T) {
	t.Logf("When data is View but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int64View(ctx, 2*time.Millisecond, func(item int64) {}, incoming)
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		outgoing := services.Int64View(ctx, 10*time.Millisecond, func(item int64) {
			inview <- item
		}, incoming)
...
func TestInt64Filter(t *testing.T) {
	t.Logf("When data is filtered but not received due to context expiration on receive")
	{
		...
		outgoing := services.Int64Filter(ctx, 2*time.Millisecond, func(item int64) bool {
			return true
		}, incoming)
		_, ok := <-outgoing
		if ok {
	...
	t.Logf("When data is filtered on each receive")
	{
		...
		incoming := make(chan int64, 0)
		outgoing := services.Int64Filter(ctx, 10*time.Millisecond, func(item int64) bool {
			return true
		}, incoming)
...
func TestInt64CollectUntil(t *testing.T) {
	t.Logf("When data is collected until condition is met except when context expiration on receive")
	{
		...
		outgoing := services.Int64CollectUntil(ctx, 2*time.Millisecond, func(item []int64) bool { return true }, incoming)
	...
	t.Logf("When data is collected until condition is met")
	{
		...
		incoming := make(chan int64, 0)
		outgoing := services.Int64CollectUntil(ctx, 10*time.Millisecond, func(items []int64) bool {
			return len(items) == 2
		}, incoming)
...
func TestInt64MergeWithoutOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64MergeWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Int64MergeWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64MergeInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64MergeInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int64MergeInOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64CombinePartiallyWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels without provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64CombinePartiallyWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels without channel order")
	{
		...
		outgoing := services.Int64CombinePartiallyWithoutOrder(ctx, 1*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64CombineWithoutOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in incoming order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64CombineWithoutOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in data incoming order")
	{
		...
		outgoing := services.Int64CombineWithoutOrder(ctx, 10*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64CombineInOrder(t *testing.T) {
	t.Logf("When data is merged from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64CombineInOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is merged from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int64CombineInOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64CombineInPartialOrder(t *testing.T) {
	t.Logf("When data is combined from multiple channels in provided order but context expires so nothing is received")
	{
		...
		outgoing := services.Int64CombineInPartialOrder(ctx, 2*time.Millisecond, incoming)
	...
	t.Logf("When data is combined from multiple channels in provided channel order")
	{
		...
		outgoing := services.Int64CombineInPartialOrder(ctx, 2*time.Millisecond, incoming, incoming2, incoming3)
...
func TestInt64Distributor(t *testing.T) {
	...
	case initial := <-incoming:
		...
		if !isInt64Equal(initial, <-incoming2) {
		...
		if !isInt64Equal(initial, <-incoming3) {
...
func isInt64Equal(item1, item2 int64) bool {
...
func isInt64EqualSlice(item1 []int64, item2 []int64) bool {
	if len(item1) != len(item2) {
		return false
	}

	for index, item := range item1 {
		if !isInt64Equal(item, item2[index]) {
			return false
		}
	}
