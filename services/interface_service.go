...
// method.
func InterfaceSink(ctx CancelContext, waitTime time.Duration, in chan<- interface{}) chan<- interface{} {
	res := make(chan interface{}, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func InterfaceSinkFilter(ctx CancelContext, waitTime time.Duration, filterFn func(interface{}) bool, in chan<- interface{}) chan<- interface{} {
	res := make(chan interface{}, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func InterfaceSinkMutate(ctx CancelContext, waitTime time.Duration, mutateFn func(interface{}) interface{}, in chan<- interface{}) chan<- interface{} {
	res := make(chan interface{}, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func InterfaceSinkView(ctx CancelContext, waitTime time.Duration, viewFn func(interface{}), in chan<- interface{}) chan<- interface{} {
	res := make(chan interface{}, 0)
...
func NewInterfaceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *InterfaceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	return &InterfaceDistributor{
...
func (d *InterfaceDistributor) Start() {
	if atomic.LoadInt64(&d.running) != 0 {
		return
	}
