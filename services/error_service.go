...
// ErrorDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type ErrorDataWriterFunc func(error) bool

// ErrorFromByteAdapter defines a function that that will take a channel of bytes and return a channel of error.
type ErrorFromByteAdapter func(CancelContext, <-chan []byte) <-chan error
...
// ErrorCollect defines a function which returns a channel where the items of the incoming channel
...
// ErrorMutate defines a function which returns a channel where the items of the incoming channel
...
// ErrorView defines a function which returns a channel where the items of the incoming channel
...
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func ErrorView(ctx CancelContext, waitTime time.Duration, viewFn func(error), in <-chan error) <-chan error {
	res := make(chan error, 0)
...
// ErrorSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func ErrorSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- error) chan<- error {
	res := make(chan error, 0)
...
// ErrorWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func ErrorWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- error) ErrorDataWriterFunc {
...
// ErrorWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func ErrorWriterFuncTo(ctx CancelContext, in chan<- error) ErrorDataWriterFunc {
	return func(val error) bool {
		select {
		case <-ctx.Done():
			return false
...
// ErrorSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func ErrorSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func(error) bool, in chan<- error) chan<- error {
	res := make(chan error, 0)
...
// ErrorSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func ErrorSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func(error) error, in chan<- error) chan<- error {
	res := make(chan error, 0)
...
// ErrorSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func ErrorSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func(error), in chan<- error) chan<- error {
	res := make(chan error, 0)
...
// ErrorFilter defines a function which returns a channel where the items of the incoming channel
...
// ErrorCollectUntil defines a function which returns a channel where the items of the incoming channel
...
// ErrorMergeWithoutOrder merges the incoming data from the error into a single stream of error,
...
// ErrorMergeInOrder merges the incoming data from the error into a single stream of error,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// ErrorCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// ErrorCombineParitallyWithoutOrder makes the following guarantees:
...
// ErrorCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// ErrorCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// ErrorCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// ErrorCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// ErrorCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// ErrorDistributor delivers messages to subscription channels which it manages internal,
...
func (d *ErrorDistributor) PublishDeadline(message error, dur time.Duration) {
...
func (d *ErrorDistributor) Publish(message error) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *ErrorDistributor) Subscribe(sub chan<- error) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *ErrorDistributor) manage() {
...
// MonoErrorService defines a interface for underline systems which want to communicate like
...
// @iface
type MonoErrorService interface {
...
// ErrorService defines a interface for underline systems which want to communicate like
