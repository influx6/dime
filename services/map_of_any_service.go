...
func MapOfAnyMergeWithoutOrder(ctx context.Context, maxWaitTime time.Duration, senders ...chan map[interface{}]interface{}) chan []map[interface{}]interface{} {
	...
	go func() {
		...
		total := len(senders)
		filled := make(map[int]map[interface{}]interface{}, 0)
...
type MonoMapOfAnyService interface {
	...
	Send() (<-chan map[interface{}]interface{}, error)
	...
	Receive(<-chan map[interface{}]interface{}) error
...
type MapOfAnyService interface {
	// Send will return a channel which will allow reading from the Service it till it is closed.
	Send(string) (<-chan map[interface{}]interface{}, error)

	// Receive will take the channel, which will be written into the Service for it's internal processing
	// and the Service will continue to read form the channel till the channel is closed.
	// Useful for collating/collecting services.
	Receive(string, <-chan map[interface{}]interface{}) error
