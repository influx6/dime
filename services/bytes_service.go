...
// method.
func BytesSink(ctx CancelContext, waitTime time.Duration, in chan<- []byte) chan<- []byte {
	res := make(chan []byte, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func BytesSinkFilter(ctx CancelContext, waitTime time.Duration, filterFn func([]byte) bool, in chan<- []byte) chan<- []byte {
	res := make(chan []byte, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func BytesSinkMutate(ctx CancelContext, waitTime time.Duration, mutateFn func([]byte) []byte, in chan<- []byte) chan<- []byte {
	res := make(chan []byte, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func BytesSinkView(ctx CancelContext, waitTime time.Duration, viewFn func([]byte), in chan<- []byte) chan<- []byte {
	res := make(chan []byte, 0)
