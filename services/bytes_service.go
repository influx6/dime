...
// BytesDataWriterFunc defines a function type which recieves a value to be written and returns true/false
// if the operation succeeded.
type BytesDataWriterFunc func([]byte) bool
...
// BytesSink defines a function which returns a channel, where the items of the returned channel
...
// method.
func BytesSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []byte) chan<- []byte {
	res := make(chan []byte, 0)
...
// BytesWriterFuncToWithin defines a function which recieves a CancelContext, max time.Duration and write channel,
...
func BytesWriterFuncToWithin(ctx CancelContext, acceptanceMaxWait time.Duration, in chan<- []byte) BytesDataWriterFunc {
...
// BytesWriterFuncTo defines a function which recieves a CancelContext and write channel, to
...
// The returned function returns true/false to signal success write of value.
func BytesWriterFuncTo(ctx CancelContext, in chan<- []byte) BytesDataWriterFunc {
	return func(val []byte) bool {
		select {
		case <-ctx.Done():
			return false
...
// BytesSinkFilter defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func BytesSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]byte) bool, in chan<- []byte) chan<- []byte {
	res := make(chan []byte, 0)
...
// BytesSinkMutate defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func BytesSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]byte) []byte, in chan<- []byte) chan<- []byte {
	res := make(chan []byte, 0)
...
// BytesSinkView defines a function which returns a channel where the items of the returned channel
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func BytesSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]byte), in chan<- []byte) chan<- []byte {
	res := make(chan []byte, 0)
