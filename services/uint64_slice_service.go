...
// UInt64SliceToByteAdapter defines a function that that will take a channel of bytes and return a channel of []uint64.
type UInt64SliceToByteAdapter func(CancelContext, <-chan []uint64) <-chan []byte

// UInt64SlicePartialCollect defines a function which returns a channel where the items of the incoming channel
...
func UInt64SlicePartialCollect(ctx CancelContext, waitTime time.Duration, in <-chan []uint64) <-chan [][]uint64 {
...
// UInt64SliceCollect defines a function which returns a channel where the items of the incoming channel
...
func UInt64SliceCollect(ctx CancelContext, waitTime time.Duration, in <-chan []uint64) <-chan [][]uint64 {
	res := make(chan [][]uint64, 0)
...
// UInt64SliceMutate defines a function which returns a channel where the items of the incoming channel
...
// This function guarantees complete data.
func UInt64SliceMutate(ctx CancelContext, waitTime time.Duration, mutateFn func([]uint64) []uint64, in <-chan []uint64) <-chan []uint64 {
	res := make(chan []uint64, 0)
...
// method.
func UInt64SliceSink(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, in chan<- []uint64) chan<- []uint64 {
	res := make(chan []uint64, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt64SliceSinkFilter(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, filterFn func([]uint64) bool, in chan<- []uint64) chan<- []uint64 {
	res := make(chan []uint64, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt64SliceSinkMutate(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, mutateFn func([]uint64) []uint64, in chan<- []uint64) chan<- []uint64 {
	res := make(chan []uint64, 0)
...
// sends data into the returned channel to ensure that it is closed and stopped once context has expired by it's Done()
// method.
func UInt64SliceSinkView(ctx CancelContext, closeInputAlso bool, waitTime time.Duration, viewFn func([]uint64), in chan<- []uint64) chan<- []uint64 {
	res := make(chan []uint64, 0)
...
// UInt64SliceFilter defines a function which returns a channel where the items of the incoming channel
// are filtered based on a function, till the provided channel is closed.
// If the given channel is closed or if the context expires, the returning channel is closed as well.
// This function guarantees complete data.
func UInt64SliceFilter(ctx CancelContext, waitTime time.Duration, filterFn func([]uint64) bool, in <-chan []uint64) <-chan []uint64 {
	res := make(chan []uint64, 0)
...
// UInt64SliceCollectUntil defines a function which returns a channel where the items of the incoming channel
...
func UInt64SliceCollectUntil(ctx CancelContext, waitTime time.Duration, condition func([][]uint64) bool, in <-chan []uint64) <-chan [][]uint64 {
	res := make(chan [][]uint64, 0)
...
// UInt64SliceMergeWithoutOrder merges the incoming data from the []uint64 into a single stream of []uint64,
...
func UInt64SliceMergeWithoutOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan []uint64) <-chan []uint64 {
	res := make(chan []uint64, 0)
...
// UInt64SliceMergeInOrder merges the incoming data from the []uint64 into a single stream of []uint64,
// merge collects data from all provided channels in turns, each giving a specified time to deliver, else
// skipped until another turn. Once all data is collected from each sender, then the data set is merged into
// a single slice and delivered to the returned channel.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt64SliceMergeInOrder(ctx CancelContext, maxWaitTime time.Duration, senders ...<-chan []uint64) <-chan []uint64 {
	res := make(chan []uint64, 0)
...
// UInt64SliceCombineParitallyWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// UInt64SliceCombineParitallyWithoutOrder makes the following guarantees:
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt64SliceCombinePartiallyWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []uint64) <-chan [][]uint64 {
	res := make(chan [][]uint64, 0)
...
// UInt64SliceCombineWithoutOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// UInt64SliceCombineWithoutOrder makes the following guarantees:
// 1. Items will be received in any order received from the channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
// 3. If any channel is closed then all operation is stopped and the returned channel is closed.
// 4. If the context expires before items are complete then returned channel is closed.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt64SliceCombineWithoutOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []uint64) <-chan [][]uint64 {
	res := make(chan [][]uint64, 0)
...
// UInt64SliceCombineInPartialOrder receives a giving stream of content from multiple channels, returning a single channel of a
// 2d slice, it sequentially tries to recieve data from each sender within a provided time duration, else skipping until it's next turn.
// This ensures every sender has adquate time to receive and reducing long blocked waits for a specific sender, more so, this reduces the
// overhead of managing multiple go-routined receving channels which are prone to goroutine ophaning or memory leaks.
// UInt64SliceCombineInPartialOrder makes the following guarantees:
// 1. Items will be received in order and return in order of channels provided.
// 2. Returned channel will never return incomplete data where a channels return value is missing from its index.
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt64SliceCombineInPartialOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []uint64) <-chan [][]uint64 {
	res := make(chan [][]uint64, 0)
...
// UInt64SliceCombineInOrder receives a giving stream of content from multiple channels, returning a single channel of a
...
// 7. Will continue to gather data from provided channels until all are closed or the context has expired.
// 8. If any of the senders is nil then the returned channel will be closed, has this leaves things in an unstable state.
func UInt64SliceCombineInOrder(ctx CancelContext, maxItemWait time.Duration, senders ...<-chan []uint64) <-chan [][]uint64 {
	res := make(chan [][]uint64, 0)
...
// UInt64SliceDistributor delivers messages to subscription channels which it manages internal,
...
type UInt64SliceDistributor struct {
	running             int64
...
// NewUInt64SliceDistributor returns a new instance of a UInt64SliceDistributor.
func NewUInt64SliceDistributor(buffer int, sendWaitBeforeAbort time.Duration) *UInt64SliceDistributor {
	if sendWaitBeforeAbort <= 0 {
		sendWaitBeforeAbort = defaultSendWithBeforeAbort
	}

	dist := &UInt64SliceDistributor{
...
func (d *UInt64SliceDistributor) PublishDeadline(message []uint64, dur time.Duration) {
...
func (d *UInt64SliceDistributor) Publish(message []uint64) {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *UInt64SliceDistributor) Clear() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
func (d *UInt64SliceDistributor) Stop() {
	if atomic.LoadInt64(&d.running) == 0 {
		return
	}
...
// subscriber adding
func (d *UInt64SliceDistributor) manage() {
...
// MonoUInt64SliceService defines a interface for underline systems which want to communicate like
...
// @iface
type MonoUInt64SliceService interface {
...
// UInt64SliceService defines a interface for underline systems which want to communicate like
// in a stream using channels to send/recieve []uint64 values. It allows different services to create adapters to
// transform data coming in and out from the Service.
// Auto-Generated using the moz code-generator https://github.com/influx6/moz.
// @iface
type UInt64SliceService interface {
	// ReadErrors will return a channel which will allow reading errors from the Service until it it is closed.
	ReadErrors() <-chan error
